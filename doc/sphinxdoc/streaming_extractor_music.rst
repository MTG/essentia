...
Specify whether you want to compute high-level descriptors based on classifier models (not computed by default) ::
...
In the profile example below, the extractor is set to analyze only the first 30 seconds of audio, output frame values as well as their statistical summarization, and apply two high-level models associated with the respective filepaths. ::
