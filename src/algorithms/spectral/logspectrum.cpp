...
void LogSpectrum::configure()
{
  ...
  _nOctaves = parameter("nOctaves").toInt();             //7;
  _nNote = _nOctaves * 12 * _nBPS + 2 * (_nBPS / 2 + 1); // a core over all octaves, plus some overlap at top and bottom
  initialize();
}
...
void LogSpectrum::compute()
{
  ...
  {
    if (spectrum[iBin] > _frameSize * 1.0)
      spectrum[iBin] = _frameSize;
    if (maxmag < spectrum[iBin])
      maxmag = spectrum[iBin];
...
void LogSpectrum::initialize()
{
  ...
  logFreqMatrix(_sampleRate, _frameSize, _nOctaves, tempkernel);
  _kernelValue.clear();
