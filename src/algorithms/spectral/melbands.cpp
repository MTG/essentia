...
void MelBands::calculateFilterFrequencies() {
  ...
  Real lowMelFrequencyBound = scale(parameter("lowFrequencyBound").toReal());
  Real highMelFrequencyBound = scale(parameter("highFrequencyBound").toReal());
  Real melFrequencyIncrement = (highMelFrequencyBound - lowMelFrequencyBound)/(filterSize + 1);

  Real melFreq = lowMelFrequencyBound;
  for (int i=0; i<filterSize + 2; ++i) {
    _filterFrequencies[i] = scale(melFreq);
...
void MelBands::createFilters(int spectrumSize) {
  ...
  */
  ...
  for (int i=0; i<filterSize; ++i) {
    Real fstep1 = scale(_filterFrequencies[i+1]) - scale(_filterFrequencies[i]);
    Real fstep2 = scale(_filterFrequencies[i+2]) - scale(_filterFrequencies[i+1]);
    ...
    for (int j=jbegin; j<jend; ++j) {
      ...
      if ((binfreq >= _filterFrequencies[i]) && (binfreq < _filterFrequencies[i+1])) {
        _filterCoefficients[i][j] = (scale(binfreq) - scale(_filterFrequencies[i])) / fstep1;
      }
      // in the descending part of the triangle...
      else if ((binfreq >= _filterFrequencies[i+1]) && (binfreq < _filterFrequencies[i+2])) {
        _filterCoefficients[i][j] = (scale(_filterFrequencies[i+2]) - scale(binfreq)) / fstep2;
      }
    }
  }
...
Real MelBands::scale(Real hz){
  Real scaled;

  if (_scale.compare("mel") == 0 ) scaled = hz2mel(hz);
  if (_scale.compare("htk") == 0 ) scaled = hz2mel10(hz);
