...
#ifndef ESSENTIA_TRIBANDS_H
#define ESSENTIA_TRIBANDS_H
...
class Triangularbands : public Algorithm {
 ...
 public:
  ...
  void declareParameters() {
    declareParameter("sampleRate", "the sampling rate of the audio signal [Hz] assuming that spectrum input is already cut at Nyquist frequency (essentia way)", "(0,inf)", 44100.);
    ...
    declareParameter("frequencyBands", "list of frequency ranges in to which the spectrum is divided (these must be in ascending order and connot contain duplicates),"\
	...
	declareParameter("Log","taking log10 (1+ magnitude) in each band","{false,true}",true);
