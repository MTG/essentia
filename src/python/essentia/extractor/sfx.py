...
def compute(audio, pool, options):
    ...
    # pitch algorithm
    pitch_detection = sonoria.PitchDetection(frameSize = 2048, sampleRate = sampleRate)

    # sfx descriptors
    spectral_peaks = sonoria.SpectralPeaks(sampleRate = sampleRate, orderBy = 'frequency')
    harmonic_peaks = sonoria.HarmonicPeaks()
    inharmonicity = sonoria.Inharmonicity()
    odd2evenharmonicenergyratio = sonoria.OddToEvenHarmonicEnergyRatio()
    tristimulus = sonoria.Tristimulus()
    ...
    envelope = sonoria.Envelope()
    file_envelope = envelope(audio)

    # temporal statistics
    decrease = sonoria.AudioDecrease(blockSize = len(audio))
    pool.add(namespace + '.' + 'temporal_decrease', decrease(file_envelope))#, pool.GlobalScope)

    centralmoments = sonoria.AudioCentralMoments(blockSize = len(audio))
    ...
    if len(pitch) > 1:
       ...
       min_to_total = sonoria.MinToTotal()
       pool.add(namespace + '.' + 'pitch_min_to_total', min_to_total(pitch))#, pool.GlobalScope)

       pitch_centroid = sonoria.Centroid(range = len(pitch)-1)
       pool.add(namespace + '.' + 'pitch_centroid', pitch_centroid(pitch))#, pool.GlobalScope)

       pitch_after_max_to_before_max_energy_ratio = sonoria.AfterMaxToBeforeMaxEnergyRatio()
