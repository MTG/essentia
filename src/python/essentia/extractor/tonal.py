...
def compute(audio, pool, options):
    ...
    spectrum = sonoria.Spectrum(size = (frameSize + zeroPadding) / 2)
    spectral_peaks = sonoria.SpectralPeaks(maxPeaks = 10000, magnitudeThreshold = 0.00001, minFrequency = 40, maxFrequency = 5000, orderBy = "frequency")
    tuning = sonoria.TuningFrequency()
    ...
    # computing the HPCPs
    spectral_whitening = sonoria.SpectralWhitening()

    hpcp_key_size = 36
    hpcp_chord_size = 36
    hpcp_tuning_size = 120

    hpcp_key = sonoria.HPCP(size = hpcp_key_size,
    ...
    hpcp_chord = sonoria.HPCP(size = hpcp_chord_size,
                               referenceFrequency = tuning_frequency,
                               harmonics = 8,
                               bandPreset = True,
                               minFrequency = 40.0,
    ...
    hpcp_tuning = sonoria.HPCP(size = hpcp_tuning_size,
                                referenceFrequency = tuning_frequency,
                                harmonics = 8,
                                bandPreset = True,
                                minFrequency = 40.0,
    ...
    for frame in frames:
        ...
        if (len(frame_frequencies) > 0):
           ...
           frame_hpcp_key = sonoria.array([0] * hpcp_key_size)
           frame_hpcp_chord = sonoria.array([0] * hpcp_chord_size)
           frame_hpcp_tuning = sonoria.array([0] * hpcp_tuning_size)
    ...
    # key detection
    key_detector = sonoria.Key(profileType = 'temperley')
    average_hpcps_key = numpy.average(sonoria.array(hpcps_key), axis=0)
    ...
    (key, scale, key_strength, first_to_second_relative_strength) = key_detector(sonoria.array(average_hpcps_key))
    ...
    # chord detection
    chord_detector = sonoria.Key(profileType = 'tonictriad', usePolyphony = False)
    ...
    for hpcp_index in range(len(hpcps_chord)):
        ...
        average_hpcps_chord = numpy.average(sonoria.array(hpcps_chord[hpcp_index_begin : hpcp_index_end]), axis=0)
        average_hpcps_chord = normalize(average_hpcps_chord)
        (key, scale, strength, first_to_second_relative_strength) = chord_detector(sonoria.array(average_hpcps_chord))
    ...
    keydetector	= sonoria.Key(profileType = 'diatonic')
    average_hpcps_tuning = numpy.average(sonoria.array(hpcps_tuning), axis=0)
    average_hpcps_tuning = normalize(average_hpcps_tuning)
    (key, scale, diatonic_strength, first_to_second_relative_strength) = keydetector(sonoria.array(average_hpcps_tuning))
    ...
    (equal_tempered_deviation,
     ...
     nontempered_peaks_energy_ratio) = sonoria.HighResolutionFeatures()(average_hpcps_tuning)
