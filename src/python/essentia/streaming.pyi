...
class AfterMaxToBeforeMaxEnergyRatio(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, pitch:float) -> float:
		"""compute
		Args:
			pitch (float): the array of pitch values [Hz]. Defaults to . 
		Returns:
...
class AllPass(_essentia.Algorithm): 
	def __init__(self, bandwidth:float=500, cutoffFrequency:float=1500, order:int=1, sampleRate:float=44100) -> None:
		...
		Args:
			bandwidth (float): the bandwidth of the filter [Hz] (used only for 2nd-order filters). Defaults to 500. Range (0,inf)
			cutoffFrequency (float): the cutoff frequency for the filter [Hz]. Defaults to 1500. Range (0,inf)
			order (int): the order of the filter. Defaults to 1. Range {1,2}
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			signal (float): the filtered signal
		""" 
		... 


class AudioLoader(_essentia.Algorithm): 
	def __init__(self, audioStream:int=0, computeMD5:bool=False, filename:str='None') -> None:
		...
		Args:
			audioStream (int): audio stream index to be loaded. Other streams are not taken into account (e.g. if stream 0 is video and 1 is audio use index 0 to access it.). Defaults to 0. Range [0,inf)
		...
		""" 
		... 
	def __call__(self, ) -> tuple[ndarray, float, int, str, int, str]:
		"""compute
		Args:
...
class AudioOnsetsMarker(_essentia.Algorithm): 
	def __init__(self, onsets:list[Any]=[], sampleRate:float=44100, type:str='beep') -> None:
		...
		Args:
			onsets (list[Any]): the list of onset locations [s]. Defaults to []. Range None
			sampleRate (float): the sampling rate of the output signal [Hz]. Defaults to 44100. Range (0,inf)
	...
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			signal (float): the input signal mixed with bursts at onset locations
		""" 
		... 


class AudioWriter(_essentia.Algorithm): 
	def __init__(self, bitrate:int=192, filename:str='None', format:str='wav', sampleRate:float=44100) -> None:
		...
		Args:
			...
			sampleRate (float): the audio sampling rate [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, audio:ndarray) -> None:
		"""compute
		Args:
			audio (ndarray): the input audio. Defaults to . 
		Returns:
None
		""" 
		... 
...
class AutoCorrelation(_essentia.Algorithm): 
	...
	def __call__(self, array:list[Any]) -> list[Any]:
		"""compute
		Args:
			array (list[Any]): the array to be analyzed. Defaults to . 
		Returns:
			autoCorrelation (list[Any]): the autocorrelation vector
		""" 
		... 
...
class BFCC(_essentia.Algorithm): 
	def __init__(self, dctType:int=2, highFrequencyBound:float=11000, inputSize:int=1025, liftering:int=0, logType:str='dbamp', lowFrequencyBound:float=0, normalize:str='unit_sum', numberBands:int=40, numberCoefficients:int=13, sampleRate:float=44100, type:str='power', weighting:str='warping') -> None:
		...
		Args:
			...
			highFrequencyBound (float): the upper bound of the frequency range [Hz]. Defaults to 11000. Range (0,inf)
			...
			lowFrequencyBound (float): the lower bound of the frequency range [Hz]. Defaults to 0. Range [0,inf)
		...
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			spectrum (list[Any]): the audio spectrum. Defaults to . 
		Returns:
			bands (list[Any]): the energies in bark bands
			bfcc (list[Any]): the bark frequency cepstrum coefficients
		""" 
		... 


class BPF(_essentia.Algorithm): 
	def __init__(self, xPoints:list[Any]=[0, 1], yPoints:list[Any]=[0, 1]) -> None:
		...
		Args:
			xPoints (list[Any]): the x-coordinates of the points forming the break-point function (the points must be arranged in ascending order and cannot contain duplicates). Defaults to [0, 1]. Range None
			yPoints (list[Any]): the y-coordinates of the points forming the break-point function. Defaults to [0, 1]. Range None 
		""" 
		... 
	def __call__(self, x:float) -> float:
		"""compute
		Args:
			x (float): the input coordinate (x-axis). Defaults to . 
		Returns:
...
class BandPass(_essentia.Algorithm): 
	def __init__(self, bandwidth:float=500, cutoffFrequency:float=1500, sampleRate:float=44100) -> None:
		...
		Args:
			bandwidth (float): the bandwidth of the filter [Hz]. Defaults to 500. Range (0,inf)
			cutoffFrequency (float): the cutoff frequency for the filter [Hz]. Defaults to 1500. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			signal (float): the filtered signal
		""" 
		... 
...
class BarkBands(_essentia.Algorithm): 
	def __init__(self, numberBands:int=27, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> list[Any]:
		"""compute
		Args:
			spectrum (list[Any]): the input spectrum. Defaults to . 
		Returns:
			bands (list[Any]): the energy of the bark bands
		""" 
		... 


class BarkExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, hopSize:int=1024, sampleRate:float=44100) -> None:
		...
		Args:
			...
			sampleRate (float): the audio sampling rate. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> tuple[list[Any], float, float, float, float, float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			barkbands (list[Any]): spectral energy at each bark band. See BarkBands alogithm
			barkbands_kurtosis (float): kurtosis from bark bands. See DistributionShape algorithm documentation
			barkbands_skewness (float): skewness from bark bands. See DistributionShape algorithm documentation
			barkbands_spread (float): spread from barkbands. See DistributionShape algorithm documentation
			spectral_crest (float): See Crest algorithm documentation
			spectral_flatness_db (float): See flatnessDB algorithm documentation
		""" 
		... 
...
class BeatTrackerDegara(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): input signal. Defaults to . 
		Returns:
			ticks (float): the estimated tick locations [s]
		""" 
		... 
...
class BeatTrackerMultiFeature(_essentia.Algorithm): 
	def __init__(self, maxTempo:int=208, minTempo:int=40) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, float]:
		"""compute
		Args:
			signal (float): input signal. Defaults to . 
		Returns:
...
class Beatogram(_essentia.Algorithm): 
	def __init__(self, size:int=16) -> None:
		...
		""" 
		... 
	def __call__(self, loudness:list[Any], loudnessBandRatio:ndarray) -> ndarray:
		"""compute
		Args:
			loudness (list[Any]): the loudness at each beat. Defaults to . 
			loudnessBandRatio (ndarray): matrix of loudness ratios at each band and beat. Defaults to . 
		Returns:
			beatogram (ndarray): filtered matrix loudness
		""" 
		... 


class BeatsLoudness(_essentia.Algorithm): 
	def __init__(self, beatDuration:float=0.05, beatWindowDuration:float=0.1, beats:list[Any]=[], frequencyBands:list[Any]=[0, 200, 400, 800, 1600, 3200, 22000], sampleRate:float=44100) -> None:
		...
		Args:
			beatDuration (float): window size for the beats' energy computation (the windows start at the onset)[s]. Defaults to 0.05. Range (0,inf)
			beatWindowDuration (float): window size for the beats' onset detection (the windows are centered around the positions in 'beats') [s]. Defaults to 0.1. Range (0,inf)
			beats (list[Any]): the list of beat positions (each position is in seconds). Defaults to []. Range None
			frequencyBands (list[Any]): the list of frequency bands to compute energy ratios [Hz]. Defaults to [0, 200, 400, 800, 1600, 3200, 22000]. Range None
			sampleRate (float): the audio sampling rate [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, list[Any]]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			loudness (float): the beat's energy in the whole spectrum
			loudnessBandRatio (list[Any]): the ratio of the beat's energy in each band
		""" 
		... 
...
class BinaryOperator(_essentia.Algorithm): 
	def __init__(self, type:str='add') -> None:
		...
		""" 
		... 
	def __call__(self, array1:list[Any], array2:list[Any]) -> list[Any]:
		"""compute
		Args:
			array1 (list[Any]): the first operand input array. Defaults to . 
			array2 (list[Any]): the second operand input array. Defaults to . 
		Returns:
			array (list[Any]): the array containing the result of binary operation
		""" 
		... 
...
class BinaryOperatorStream(_essentia.Algorithm): 
	def __init__(self, type:str='add') -> None:
		...
		""" 
		... 
	def __call__(self, array1:float, array2:float) -> float:
		"""compute
		Args:
			array1 (float): the first operand input array. Defaults to . 
			array2 (float): the second operand input array. Defaults to . 
		Returns:
			array (float): the array containing the result of binary operation
		""" 
		... 
...
class BpmHistogram(_essentia.Algorithm): 
	def __init__(self, bpm:float=0, constantTempo:bool=False, frameRate:float=86.1328, frameSize:float=4, maxBpm:float=560, maxPeaks:int=50, minBpm:float=30, overlap:int=16, tempoChange:float=5, weightByMagnitude:bool=True, windowType:str='hann', zeroPadding:int=0) -> None:
		...
		Args:
			bpm (float): bpm to induce a certain tempo tracking. Zero if unknown. Defaults to 0. Range [0,inf)
			...
			frameSize (float): the minimum length to compute the FFT [s]. Defaults to 4. Range [1,inf)
			maxBpm (float): the maximum bpm to consider. Defaults to 560. Range (0,inf)
			maxPeaks (int): the number of peaks to be considered at each spectrum. Defaults to 50. Range (0,inf]
			minBpm (float): the minimum bpm to consider. Defaults to 30. Range [0,inf)
			overlap (int): the overlap factor. Defaults to 16. Range (0,inf)
			tempoChange (float): the minimum length to consider a change in tempo as stable [s]. Defaults to 5. Range [0,inf)
		...
		""" 
		... 
	def __call__(self, novelty:float) -> tuple[float, list[Any], list[Any], ndarray, list[Any], list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			novelty (float): the novelty curve. Defaults to . 
		Returns:
			bpm (float): mean BPM of the most salient tempo
			bpmCandidates (list[Any]): list of the most salient BPM values
			bpmMagnitudes (list[Any]): magnitudes of the most salient BPM values
			tempogram (ndarray): spectrogram-like representation of tempo over time (frames of BPM magnitudes)
			frameBpms (list[Any]): BPM values at each frame
			ticks (list[Any]): time positions of ticks [s]
			ticksMagnitude (list[Any]): ticks' strength (magnitude)
			sinusoid (list[Any]): sinusoid whose peaks indicate tick positions
		""" 
		... 
...
class BpmHistogramDescriptors(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, bpmIntervals:list[Any]) -> tuple[float, float, float, float, float, float, list[Any]]:
		"""compute
		Args:
			bpmIntervals (list[Any]): the list of bpm intervals [s]. Defaults to . 
		Returns:
			...
			histogram (list[Any]): bpm histogram [bpm]
		""" 
		... 
...
class BpmRubato(_essentia.Algorithm): 
	def __init__(self, longRegionsPruningTime:float=20, shortRegionsMergingTime:float=4, tolerance:float=0.08) -> None:
		...
		Args:
			longRegionsPruningTime (float): time for the longest constant tempo region inside a rubato region [s]. Defaults to 20. Range [0,inf)
			shortRegionsMergingTime (float): time for the shortest constant tempo region from one tempo region to another [s]. Defaults to 4. Range [0,inf)
			tolerance (float): minimum tempo deviation to look for. Defaults to 0.08. Range [0,1] 
		""" 
		... 
	def __call__(self, beats:list[Any]) -> tuple[list[Any], list[Any], int]:
		"""compute
		Args:
			beats (list[Any]): list of detected beat ticks [s]. Defaults to . 
		Returns:
			rubatoStart (list[Any]): list of timestamps where the start of a rubato region was detected [s]
			rubatoStop (list[Any]): list of timestamps where the end of a rubato region was detected [s]
...
class CartesianToPolar(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, complex:ndarray) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			complex (ndarray): the complex input vector. Defaults to . 
		Returns:
			magnitude (list[Any]): the magnitude vector
			phase (list[Any]): the phase vector
		""" 
		... 
...
class CentralMoments(_essentia.Algorithm): 
	def __init__(self, mode:str='pdf', range:float=1) -> None:
		...
		Args:
			...
			range (float): the range of the input array, used for normalizing the results in the 'pdf' mode. Defaults to 1. Range (0,inf) 
		""" 
		... 
	def __call__(self, array:list[Any]) -> list[Any]:
		"""compute
		Args:
			array (list[Any]): the input array. Defaults to . 
		Returns:
			centralMoments (list[Any]): the central moments of the input array
		""" 
		... 
...
class Centroid(_essentia.Algorithm): 
	def __init__(self, range:float=1) -> None:
		...
		Args:
			range (float): the range of the input array, used for normalizing the results. Defaults to 1. Range (0,inf) 
		""" 
		... 
	def __call__(self, array:list[Any]) -> float:
		"""compute
		Args:
			array (list[Any]): the input array. Defaults to . 
		Returns:
...
class ChordsDescriptors(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, chords:str, key:str, scale:str) -> tuple[list[Any], float, float, str, str]:
		"""compute
		Args:
			chords (str): the chord progression. Defaults to . 
			key (str): the key of the whole song, from A to G. Defaults to . 
			scale (str): the scale of the whole song (major or minor). Defaults to . 
		Returns:
			chordsHistogram (list[Any]): the normalized histogram of chords
			...
			chordsKey (str): the key of the progression, taken as the most frequent chord
			chordsScale (str): the scale of the progression, whether major or minor
		""" 
		... 
...
class ChordsDetection(_essentia.Algorithm): 
	def __init__(self, hopSize:int=2048, sampleRate:float=44100, windowSize:float=2) -> None:
		...
		Args:
			...
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			windowSize (float): the size of the window on which to estimate the chords [s]. Defaults to 2. Range (0,inf) 
		""" 
		... 
	def __call__(self, pcp:list[Any]) -> tuple[str, float]:
		"""compute
		Args:
			pcp (list[Any]): the pitch class profile from which to detect the chord. Defaults to . 
		Returns:
			chords (str): the resulting chords, from A to G
			strength (float): the strength of the chord
		""" 
		... 


class ChromaCrossSimilarity(_essentia.Algorithm): 
	def __init__(self, referenceFeature:ndarray|None, binarizePercentile:float=0.095, frameStackSize:int=9, frameStackStride:int=1, oti:int=0, otiBinary:bool=False) -> None:
		...
		Args:
			...
			oti (int): optimal transposition index of the query and reference song if known. Defaults to 0. Range [0,inf]
			otiBinary (bool): whether to use the OTI-based chroma binary similarity method. Defaults to False. Range {true,false}
			referenceFeature (ndarray): 2D matrix corresponds to the input chromagram of the reference song. (eg. a HPCP). Defaults to []. Range None 
		""" 
		... 
	def __call__(self, queryFeature:list[Any]) -> list[Any]:
		"""compute
		Args:
			queryFeature (list[Any]): input chromagram of the query song. (eg: a HPCP). Defaults to . 
		Returns:
			csm (list[Any]): 2D binary cross-similarity matrix of the query and reference chromagram
		""" 
		... 
...
class Chromagram(_essentia.Algorithm): 
	def __init__(self, binsPerOctave:int=12, minFrequency:float=32.7, minimumKernelSize:int=4, normalizeType:str='unit_max', numberBins:int=84, sampleRate:float=44100, scale:float=1, threshold:float=0.01, windowType:str='hann', zeroPhase:bool=True) -> None:
		...
		Args:
			...
			sampleRate (float): FFT sampling rate [Hz]. Defaults to 44100. Range [0,inf)
			scale (float): filters scale. Larger values use longer windows. Defaults to 1. Range [0,inf)
		...
		""" 
		... 
	def __call__(self, frame:list[Any]) -> list[Any]:
		"""compute
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			chromagram (list[Any]): the magnitude constant-Q chromagram
		""" 
		... 
...
class ClickDetector(_essentia.Algorithm): 
	def __init__(self, detectionThreshold:float=30, frameSize:int=512, hopSize:int=256, order:int=12, powerEstimationThreshold:int=10, sampleRate:float=44100, silenceThreshold:int=-50) -> None:
		...
		Args:
			detectionThreshold (float): 'detectionThreshold' the threshold is based on the instant power of the noisy excitation signal plus detectionThreshold dBs. Defaults to 30. Range (-inf,inf)
			...
			sampleRate (float): sample rate used for the analysis. Defaults to 44100. Range (0,inf)
			silenceThreshold (int): threshold to skip silent frames. Defaults to -50. Range (-inf,0) 
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			frame (list[Any]): the input frame (must be non-empty). Defaults to . 
		Returns:
			starts (list[Any]): starting indexes of the clicks
			ends (list[Any]): ending indexes of the clicks
		""" 
		... 
...
class Clipper(_essentia.Algorithm): 
	def __init__(self, max:float=1, min:float=-1) -> None:
		...
		Args:
			max (float): the maximum value above which the signal will be clipped. Defaults to 1. Range (-inf,inf)
			min (float): the minimum value below which the signal will be clipped. Defaults to -1. Range (-inf,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			signal (float): the output signal with the added noise
		""" 
		... 
...
class ConstantQ(_essentia.Algorithm): 
	def __init__(self, binsPerOctave:int=12, minFrequency:float=32.7, minimumKernelSize:int=4, numberBins:int=84, sampleRate:float=44100, scale:float=1, threshold:float=0.01, windowType:str='hann', zeroPhase:bool=True) -> None:
		...
		""" 
		... 
	def __call__(self, frame:list[Any]) -> ndarray:
		"""compute
		Args:
			frame (list[Any]): the windowed input audio frame. Defaults to . 
		Returns:
			constantq (ndarray): the Constant Q transform
		""" 
		... 
...
class CoverSongSimilarity(_essentia.Algorithm): 
	def __init__(self, disExtension:float=0.5, disOnset:float=0.5, distanceType:str='asymmetric', pipeDistance:bool=False) -> None:
		...
		Args:
			...
			pipeDistance (bool): whether to pipe-out the computed cover song similarity distance for each stream of input similarity matrix. Defaults to False. Range {true,false} 
		""" 
		... 
	def __call__(self, inputArray:list[Any]) -> tuple[ndarray, float]:
		"""compute
		Args:
			inputArray (list[Any]): a 2D binary cross similarity matrix of two audio chroma vectors (refer CrossSimilarityMatrix algorithm').. Defaults to . 
		Returns:
			scoreMatrix (ndarray): a 2D smith-waterman alignment score matrix from the input binary cross-similarity matrix as described in [2].
			distance (float): cover song similarity distance between the query and reference song from the input similarity. Either 'asymmetric' (as described in [2]) or 'symmetric' (maximum score in the alignment score matrix).
		""" 
		... 
...
class Crest(_essentia.Algorithm): 
	...
	def __call__(self, array:list[Any]) -> float:
		"""compute
		Args:
			array (list[Any]): the input array (cannot contain negative values, and must be non-empty). Defaults to . 
		Returns:
...
class CrossCorrelation(_essentia.Algorithm): 
	def __init__(self, maxLag:int=1, minLag:int=0) -> None:
		...
		""" 
		... 
	def __call__(self, arrayX:list[Any], arrayY:list[Any]) -> list[Any]:
		"""compute
		Args:
			arrayX (list[Any]): the first input array. Defaults to . 
			arrayY (list[Any]): the second input array. Defaults to . 
		Returns:
			crossCorrelation (list[Any]): the cross-correlation vector between the two input arrays (its size is equal to maxLag - minLag + 1)
		""" 
		... 


class CubicSpline(_essentia.Algorithm): 
	def __init__(self, leftBoundaryFlag:int=0, leftBoundaryValue:float=0, rightBoundaryFlag:int=0, rightBoundaryValue:float=0, xPoints:list[Any]=[0, 1], yPoints:list[Any]=[0, 1]) -> None:
		...
		Args:
			...
			leftBoundaryValue (float): the value to be used in the left boundary, when leftBoundaryFlag is 1 or 2. Defaults to 0. Range (-inf,inf)
			rightBoundaryFlag (int): type of boundary condition for the right boundary. Defaults to 0. Range {0,1,2}
			rightBoundaryValue (float): the value to be used in the right boundary, when rightBoundaryFlag is 1 or 2. Defaults to 0. Range (-inf,inf)
			xPoints (list[Any]): the x-coordinates where data is specified (the points must be arranged in ascending order and cannot contain duplicates). Defaults to [0, 1]. Range None
			yPoints (list[Any]): the y-coordinates to be interpolated (i.e. the known data). Defaults to [0, 1]. Range None 
		""" 
		... 
...
class DCRemoval(_essentia.Algorithm): 
	def __init__(self, cutoffFrequency:float=40, sampleRate:float=44100) -> None:
		...
		Args:
			cutoffFrequency (float): the cutoff frequency for the filter [Hz]. Defaults to 40. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			signal (float): the filtered signal, with the DC component removed
		""" 
		... 
...
class DCT(_essentia.Algorithm): 
	...
	def __call__(self, array:list[Any]) -> list[Any]:
		...
		Args:
			array (list[Any]): the input array. Defaults to . 
		Returns:
			dct (list[Any]): the discrete cosine transform of the input array
		""" 
		... 
...
class Danceability(_essentia.Algorithm): 
	def __init__(self, maxTau:float=8800, minTau:float=310, sampleRate:float=44100, tauMultiplier:float=1.1) -> None:
		...
		Args:
			maxTau (float): maximum segment length to consider [ms]. Defaults to 8800. Range (0,inf)
			minTau (float): minimum segment length to consider [ms]. Defaults to 310. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
	...
	def __call__(self, signal:float) -> tuple[float, list[Any]]:
		...
		Returns:
			...
			dfa (list[Any]): the DFA exponent vector for considered segment length (tau) values
		""" 
		... 
...
class Decrease(_essentia.Algorithm): 
	def __init__(self, range:float=1) -> None:
		...
		Args:
			range (float): the range of the input array, used for normalizing the results. Defaults to 1. Range (-inf,inf) 
		""" 
		... 
	def __call__(self, array:list[Any]) -> float:
		"""compute
...
class Derivative(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			signal (float): the derivative of the input signal
		""" 
		... 
...
class DerivativeSFX(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, envelope:list[Any]) -> tuple[float, float]:
		"""compute
		Args:
			envelope (list[Any]): the envelope of the signal. Defaults to . 
		Returns:
...
class DiscontinuityDetector(_essentia.Algorithm): 
	def __init__(self, detectionThreshold:float=8, energyThreshold:float=-60, frameSize:int=512, hopSize:int=256, kernelSize:int=7, order:int=3, silenceThreshold:int=-50, subFrameSize:int=32) -> None:
		...
		Args:
			detectionThreshold (float): 'detectionThreshold' times the standard deviation plus the median of the frame is used as detection threshold. Defaults to 8. Range [1,inf)
			energyThreshold (float): threshold in dB to detect silent subframes. Defaults to -60. Range (-inf,inf)
			frameSize (int): the expected size of the input audio signal (this is an optional parameter to optimize memory allocation). Defaults to 512. Range (0,inf)
			hopSize (int): hop size used for the analysis. This parameter must be set correctly as it cannot be obtained from the input data. Defaults to 256. Range [0,inf)
	...
	def __call__(self, frame:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Args:
			frame (list[Any]): the input frame (must be non-empty). Defaults to . 
		Returns:
			discontinuityLocations (list[Any]): the index of the detected discontinuities (if any)
			discontinuityAmplitudes (list[Any]): the peak values of the prediction error for the discontinuities (if any)
		""" 
		... 
...
class Dissonance(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, frequencies:list[Any], magnitudes:list[Any]) -> float:
		"""compute
		Args:
			frequencies (list[Any]): the frequencies of the spectral peaks (must be sorted by frequency). Defaults to . 
			magnitudes (list[Any]): the magnitudes of the spectral peaks (must be sorted by frequency. Defaults to . 
		Returns:
...
class DistributionShape(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, centralMoments:list[Any]) -> tuple[float, float, float]:
		"""compute
		Args:
			centralMoments (list[Any]): the central moments of a distribution. Defaults to . 
		Returns:
...
class ERBBands(_essentia.Algorithm): 
	def __init__(self, highFrequencyBound:float=22050, inputSize:int=1025, lowFrequencyBound:float=50, numberBands:int=40, sampleRate:float=44100, type:str='power', width:float=1) -> None:
		...
		Args:
			highFrequencyBound (float): an upper-bound limit for the frequencies to be included in the bands. Defaults to 22050. Range [0,inf)
			inputSize (int): the size of the spectrum. Defaults to 1025. Range (1,inf)
			lowFrequencyBound (float): a lower-bound limit for the frequencies to be included in the bands. Defaults to 50. Range [0,inf)
			numberBands (int): the number of output bands. Defaults to 40. Range (1,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			type (str): use magnitude or power spectrum. Defaults to 'power'. Range {magnitude,power}
			width (float): filter width with respect to ERB. Defaults to 1. Range (0,inf) 
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> list[Any]:
		"""compute
		Args:
			spectrum (list[Any]): the audio spectrum. Defaults to . 
		Returns:
			bands (list[Any]): the energies/magnitudes of each band
		""" 
		... 


class EasyLoader(_essentia.Algorithm): 
	def __init__(self, audioStream:int=0, downmix:str='mix', endTime:float=1e+06, filename:str='None', replayGain:float=-6, sampleRate:float=44100, startTime:float=0) -> None:
		...
		Args:
			...
			endTime (float): the end time of the slice to be extracted [s]. Defaults to 1e+06. Range [0,inf)
			filename (str): the name of the file from which to read. Defaults to 'None'. Range None
			replayGain (float): the value of the replayGain that should be used to normalize the signal [dB]. Defaults to -6. Range (-inf,inf)
			sampleRate (float): the output sampling rate [Hz]. Defaults to 44100. Range (0,inf)
			startTime (float): the start time of the slice to be extracted [s]. Defaults to 0. Range [0,inf) 
		""" 
		... 
	def __call__(self, ) -> float:
		...
		Returns:
			audio (float): the output audio signal
		""" 
		... 
...
class EffectiveDuration(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100, thresholdRatio:float=0.4) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> float:
		"""compute
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
...
class EnergyBand(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100, startCutoffFrequency:float=0, stopCutoffFrequency:float=100) -> None:
		...
		Args:
			sampleRate (float): the audio sampling rate [Hz]. Defaults to 44100. Range (0,inf)
			startCutoffFrequency (float): the start frequency from which to sum the energy [Hz]. Defaults to 0. Range [0,inf)
			stopCutoffFrequency (float): the stop frequency to which to sum the energy [Hz]. Defaults to 100. Range (0,inf) 
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> float:
		"""compute
		Args:
			spectrum (list[Any]): the input frequency spectrum. Defaults to . 
		Returns:
...
class EnergyBandRatio(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100, startFrequency:float=0, stopFrequency:float=100) -> None:
		...
		Args:
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			startFrequency (float): the frequency from which to start summing the energy [Hz]. Defaults to 0. Range [0,inf)
			stopFrequency (float): the frequency up to which to sum the energy [Hz]. Defaults to 100. Range [0,inf) 
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> float:
		"""compute
		Args:
			spectrum (list[Any]): the input audio spectrum. Defaults to . 
		Returns:
...
class Envelope(_essentia.Algorithm): 
	def __init__(self, applyRectification:bool=True, attackTime:float=10, releaseTime:float=1500, sampleRate:float=44100) -> None:
		...
		Args:
			...
			attackTime (float): the attack time of the first order lowpass in the attack phase [ms]. Defaults to 10. Range [0,inf)
			releaseTime (float): the release time of the first order lowpass in the release phase [ms]. Defaults to 1500. Range [0,inf)
			sampleRate (float): the audio sampling rate [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			signal (float): the resulting envelope of the signal
		""" 
		... 


class EqloudLoader(_essentia.Algorithm): 
	def __init__(self, downmix:str='mix', endTime:float=1e+06, filename:str='None', replayGain:float=-6, sampleRate:float=44100, startTime:float=0) -> None:
		...
		Args:
			...
			endTime (float): the end time of the slice to be extracted [s]. Defaults to 1e+06. Range [0,inf)
			filename (str): the name of the file from which to read. Defaults to 'None'. Range None
			replayGain (float): the value of the replayGain [dB] that should be used to normalize the signal [dB]. Defaults to -6. Range (-inf,inf)
			sampleRate (float): the output sampling rate [Hz]. Defaults to 44100. Range {32000,44100,48000}
			startTime (float): the start time of the slice to be extracted [s]. Defaults to 0. Range [0,inf) 
		""" 
		... 
	def __call__(self, ) -> float:
...
class EqualLoudness(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100) -> None:
		...
		Args:
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range {8000,16000,32000,44100,48000} 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		"""compute
...
class FFT(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> ndarray:
		...
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			fft (ndarray): the FFT of the input frame
		""" 
		... 
...
class FFTC(_essentia.Algorithm): 
	...
	def __call__(self, frame:ndarray) -> ndarray:
		"""compute
		Args:
			frame (ndarray): the input frame (complex). Defaults to . 
		Returns:
			fft (ndarray): the FFT of the input frame
		""" 
		... 
...
class FadeDetection(_essentia.Algorithm): 
	def __init__(self, cutoffHigh:float=0.85, cutoffLow:float=0.2, frameRate:float=4, minLength:float=3) -> None:
		...
		Args:
			...
			frameRate (float): the rate of frames used in calculation of the RMS [frames/s]. Defaults to 4. Range (0,inf)
			minLength (float): the minimum length to consider a fade-in/out [s]. Defaults to 3. Range (0,inf) 
		""" 
		... 
	def __call__(self, rms:float) -> tuple[ndarray, ndarray]:
		"""compute
		Args:
			rms (float): rms values array. Defaults to . 
		Returns:
			fadeIn (ndarray): 2D-array containing start/stop timestamps corresponding to fade-ins [s] (ordered chronologically)
			fadeOut (ndarray): 2D-array containing start/stop timestamps corresponding to fade-outs [s] (ordered chronologically)
		""" 
		... 
...
class FalseStereoDetector(_essentia.Algorithm): 
	def __init__(self, correlationThreshold:float=0.9995, frameSize:int=512, silenceThreshold:int=-70) -> None:
		...
		Args:
			correlationThreshold (float): silence threshold. Silent frames will be skkiped.. Defaults to 0.9995. Range [-1,1]
			frameSize (int): desired frame size for the analysis.. Defaults to 512. Range (0,inf)
			silenceThreshold (int): correation computation can be skipped if not required.. Defaults to -70. Range (-inf,0) 
		""" 
		... 
	def __call__(self, audio:ndarray) -> tuple[int, float]:
		"""compute
		Args:
			audio (ndarray): the input audio. Defaults to . 
		Returns:
...
class FileOutput(_essentia.Algorithm): 
	def __init__(self, filename:str='out.txt', mode:str='text') -> None:
		"""Data into text or binary files 

		Args:
			filename (str): the name of the output file (use '-' for stdout). Defaults to 'out.txt'. Range None
			mode (str): output mode. Defaults to 'text'. Range {text,binary} 
		""" 
		... 
...
class FlatnessSFX(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, envelope:list[Any]) -> float:
		"""compute
		Args:
			envelope (list[Any]): the envelope of the signal. Defaults to . 
		Returns:
...
class FrameBuffer(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> list[Any]:
		...
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			frame (list[Any]): the buffered audio frame
		""" 
		... 
...
class FrameCutter(_essentia.Algorithm): 
	def __init__(self, frameSize:int=1024, hopSize:int=512, lastFrameToEndOfFile:bool=False, silentFrames:str='noise', startFromZero:bool=False, validFrameThresholdRatio:float=0) -> None:
		...
		Args:
			frameSize (int): the size of the frame to cut. Defaults to 1024. Range [1,inf)
			hopSize (int): the number of samples to jump after a frame is output. Defaults to 512. Range [1,inf)
			...
			silentFrames (str): whether to [keep/drop/add noise to] silent frames. Defaults to 'noise'. Range {drop,keep,noise}
			...
			validFrameThresholdRatio (float): frames smaller than this ratio will be discarded, those larger will be zero-padded to a full frame (i.e. a value of 0 will never discard frames and a value of 1 will only keep frames that are of length 'frameSize'). Defaults to 0. Range [0,1] 
		""" 
		... 
	def __call__(self, signal:float) -> list[Any]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			frame (list[Any]): the frames of the audio signal
		""" 
		... 
...
class FrameToReal(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> float:
		"""compute
		Args:
			signal (list[Any]): the input audio frame. Defaults to . 
		Returns:
			signal (float): the output audio samples
		""" 
		... 


class FrequencyBands(_essentia.Algorithm): 
	def __init__(self, frequencyBands:list[Any]=[0, 50, 100, 150, 200, 300, 400, 510, 630, 770, 920, 1080, 1270, 1480, 1720, 2000, 2320, 2700, 3150, 3700, 4400, 5300, 6400, 7700, 9500, 12000, 15500, 20500, 27000], sampleRate:float=44100) -> None:
		...
		Args:
			frequencyBands (list[Any]): list of frequency ranges in to which the spectrum is divided (these must be in ascending order and connot contain duplicates). Defaults to [0, 50, 100, 150, 200, 300, 400, 510, 630, 770, 920, 1080, 1270, 1480, 1720, 2000, 2320, 2700, 3150, 3700, 4400, 5300, 6400, 7700, 9500, 12000, 15500, 20500, 27000]. Range None
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> list[Any]:
		"""compute
		Args:
			spectrum (list[Any]): the input spectrum (must be greater than size one). Defaults to . 
		Returns:
			bands (list[Any]): the energy in each band
		""" 
		... 
...
class GFCC(_essentia.Algorithm): 
	def __init__(self, dctType:int=2, highFrequencyBound:float=22050, inputSize:int=1025, logType:str='dbamp', lowFrequencyBound:float=40, numberBands:int=40, numberCoefficients:int=13, sampleRate:float=44100, silenceThreshold:float=1e-10, type:str='power') -> None:
		...
		Args:
			dctType (int): the DCT type. Defaults to 2. Range [2,3]
			highFrequencyBound (float): the upper bound of the frequency range [Hz]. Defaults to 22050. Range (0,inf)
			inputSize (int): the size of input spectrum. Defaults to 1025. Range (1,inf)
			logType (str): logarithmic compression type. Use 'dbpow' if working with power and 'dbamp' if working with magnitudes. Defaults to 'dbamp'. Range {natural,dbpow,dbamp,log}
			lowFrequencyBound (float): the lower bound of the frequency range [Hz]. Defaults to 40. Range [0,inf)
	...
	def __call__(self, spectrum:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Args:
			spectrum (list[Any]): the audio spectrum. Defaults to . 
		Returns:
			bands (list[Any]): the energies in ERB bands
			gfcc (list[Any]): the gammatone feature cepstrum coefficients
		""" 
		... 
...
class GapsDetector(_essentia.Algorithm): 
	def __init__(self, attackTime:float=0.05, frameSize:int=2048, hopSize:int=1024, kernelSize:int=11, maximumTime:float=3500, minimumTime:float=10, postpowerTime:float=40, prepowerThreshold:float=-30, prepowerTime:float=40, releaseTime:float=0.05, sampleRate:float=44100, silenceThreshold:float=-50) -> None:
		...
		Args:
			...
			maximumTime (float): time of the maximum gap duration [ms]. Defaults to 3500. Range (0,inf)
			minimumTime (float): time of the minimum gap duration [ms]. Defaults to 10. Range (0,inf)
			postpowerTime (float): time for the postpower calculation [ms]. Defaults to 40. Range (0,inf)
			prepowerThreshold (float): prepower threshold [dB]. . Defaults to -30. Range (-inf,inf)
			prepowerTime (float): time for the prepower calculation [ms]. Defaults to 40. Range (0,inf)
			releaseTime (float): the release time of the first order lowpass in the release phase [ms]. Defaults to 0.05. Range [0,inf)
			sampleRate (float): sample rate used for the analysis. Defaults to 44100. Range (0,inf)
			silenceThreshold (float): silence threshold [dB]. Defaults to -50. Range (-inf,inf) 
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			frame (list[Any]): the input frame (must be non-empty). Defaults to . 
		Returns:
			starts (list[Any]): the start indexes of the detected gaps (if any) in seconds
			ends (list[Any]): the end indexes of the detected gaps (if any) in seconds
		""" 
		... 
...
class HPCP(_essentia.Algorithm): 
	def __init__(self, bandPreset:bool=True, bandSplitFrequency:float=500, harmonics:int=0, maxFrequency:float=5000, maxShifted:bool=False, minFrequency:float=40, nonLinear:bool=False, normalized:str='unitMax', referenceFrequency:float=440, sampleRate:float=44100, size:int=12, weightType:str='squaredCosine', windowSize:float=1) -> None:
		...
		Args:
			...
			bandSplitFrequency (float): the split frequency for low and high bands, not used if bandPreset is false [Hz]. Defaults to 500. Range (0,inf)
			...
			maxFrequency (float): the maximum frequency that contributes to the HPCP [Hz] (the difference between the max and split frequencies must not be less than 200.0 Hz). Defaults to 5000. Range (0,inf)
			...
			minFrequency (float): the minimum frequency that contributes to the HPCP [Hz] (the difference between the min and split frequencies must not be less than 200.0 Hz). Defaults to 40. Range (0,inf)
			...
			referenceFrequency (float): the reference frequency for semitone index calculation, corresponding to A3 [Hz]. Defaults to 440. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			...
			windowSize (float): the size, in semitones, of the window used for the weighting. Defaults to 1. Range (0,12] 
		""" 
		... 
	def __call__(self, frequencies:list[Any], magnitudes:list[Any]) -> list[Any]:
		"""compute
		Args:
			frequencies (list[Any]): the frequencies of the spectral peaks [Hz]. Defaults to . 
			magnitudes (list[Any]): the magnitudes of the spectral peaks. Defaults to . 
		Returns:
			hpcp (list[Any]): the resulting harmonic pitch class profile
		""" 
		... 
...
class HarmonicBpm(_essentia.Algorithm): 
	def __init__(self, bpm:float=60, threshold:float=20, tolerance:float=5) -> None:
		...
		Args:
			bpm (float): the bpm used to find its harmonics. Defaults to 60. Range [1,inf)
			threshold (float): bpm threshold below which greatest common divisors are discarded. Defaults to 20. Range [1,inf)
			tolerance (float): percentage tolerance to consider two bpms are equal or equal to a harmonic. Defaults to 5. Range [0,inf) 
		""" 
		... 
	def __call__(self, bpms:list[Any]) -> list[Any]:
		"""compute
		Args:
			bpms (list[Any]): list of bpm candidates. Defaults to . 
		Returns:
			harmonicBpms (list[Any]): a list of bpms which are harmonically related to the bpm parameter 
		""" 
		... 
...
class HarmonicMask(_essentia.Algorithm): 
	def __init__(self, attenuation:float=-200, binWidth:int=4, sampleRate:float=44100) -> None:
		...
		Args:
			attenuation (float): attenuation in dB's of the muted pitched component. If value is positive the pitched component is attenuated (muted), if the value is negative the pitched component is soloed (i.e. background component is attenuated).. Defaults to -200. Range [-inf,inf)
		...
		""" 
		... 
	def __call__(self, fft:ndarray, pitch:float) -> ndarray:
		"""compute
		Args:
			fft (ndarray): the input frame. Defaults to . 
			pitch (float): an estimate of the fundamental frequency of the signal [Hz]. Defaults to . 
		Returns:
			fft (ndarray): the output frame
		""" 
		... 
...
class HarmonicModelAnal(_essentia.Algorithm): 
	def __init__(self, freqDevOffset:float=20, freqDevSlope:float=0.01, harmDevSlope:float=0.01, hopSize:int=512, magnitudeThreshold:float=-74, maxFrequency:float=5000, maxPeaks:int=100, maxnSines:int=100, minFrequency:float=20, nHarmonics:int=100, orderBy:str='frequency', sampleRate:float=44100) -> None:
		...
		Args:
			freqDevOffset (float): minimum frequency deviation at 0Hz. Defaults to 20. Range (0,inf)
			...
			magnitudeThreshold (float): peaks below this given threshold are not outputted. Defaults to -74. Range (-inf,inf)
			maxFrequency (float): the maximum frequency of the F0 [Hz]. Defaults to 5000. Range (0,inf)
			...
			minFrequency (float): the minimum frequency of the F0 [Hz]. Defaults to 20. Range (0,inf)
		...
		""" 
		... 
	def __call__(self, fft:ndarray, pitch:float) -> tuple[list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			fft (ndarray): the input fft. Defaults to . 
			pitch (float): external pitch input [Hz].. Defaults to . 
		Returns:
			frequencies (list[Any]): the frequencies of the sinusoidal peaks [Hz]
			magnitudes (list[Any]): the magnitudes of the sinusoidal peaks
			phases (list[Any]): the phases of the sinusoidal peaks
		""" 
		... 
...
class HarmonicPeaks(_essentia.Algorithm): 
	def __init__(self, maxHarmonics:int=20, tolerance:float=0.2) -> None:
		...
		""" 
		... 
	def __call__(self, frequencies:list[Any], magnitudes:list[Any], pitch:float) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			frequencies (list[Any]): the frequencies of the spectral peaks [Hz] (ascending order). Defaults to . 
			magnitudes (list[Any]): the magnitudes of the spectral peaks (ascending frequency order). Defaults to . 
			pitch (float): an estimate of the fundamental frequency of the signal [Hz]. Defaults to . 
		Returns:
			harmonicFrequencies (list[Any]): the frequencies of harmonic peaks [Hz]
			harmonicMagnitudes (list[Any]): the magnitudes of harmonic peaks
		""" 
		... 
...
class HighResolutionFeatures(_essentia.Algorithm): 
	def __init__(self, maxPeaks:int=24) -> None:
		...
		""" 
		... 
	def __call__(self, hpcp:list[Any]) -> tuple[float, float, float]:
		"""compute
		Args:
			hpcp (list[Any]): the pitch class profile from which to detect the chord. Defaults to . 
		Returns:
...
class Histogram(_essentia.Algorithm): 
	def __init__(self, maxValue:float=1, minValue:float=0, normalize:str='none', numberBins:int=10) -> None:
		...
		Args:
			maxValue (float): the max value of the histogram. Defaults to 1. Range [0,Inf)
			minValue (float): the min value of the histogram. Defaults to 0. Range [0,Inf)
		...
		""" 
		... 
	def __call__(self, array:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			array (list[Any]): the input array. Defaults to . 
		Returns:
			histogram (list[Any]): the values in the equally-spaced bins
			binEdges (list[Any]): the edges of the equally-spaced bins. Size is _histogram.size() + 1
		""" 
		... 
...
class HprModelAnal(_essentia.Algorithm): 
	def __init__(self, fftSize:int=2048, freqDevOffset:int=20, freqDevSlope:float=0.01, harmDevSlope:float=0.01, hopSize:int=512, magnitudeThreshold:float=0, maxFrequency:float=5000, maxPeaks:int=100, maxnSines:int=100, minFrequency:float=20, nHarmonics:int=100, orderBy:str='frequency', sampleRate:float=44100, stocf:float=0.2) -> None:
		...
		Args:
			...
			harmDevSlope (float): slope increase of minimum frequency deviation. Defaults to 0.01. Range (-inf,inf)
			hopSize (int): the hop size between frames. Defaults to 512. Range [1,inf)
			magnitudeThreshold (float): peaks below this given threshold are not outputted. Defaults to 0. Range (-inf,inf)
			maxFrequency (float): the maximum frequency of the range to evaluate [Hz]. Defaults to 5000. Range (0,inf)
			maxPeaks (int): the maximum number of returned peaks. Defaults to 100. Range [1,inf)
			maxnSines (int): maximum number of sines per frame. Defaults to 100. Range (0,inf)
			minFrequency (float): the minimum frequency of the range to evaluate [Hz]. Defaults to 20. Range (0,inf)
			nHarmonics (int): maximum number of harmonics per frame. Defaults to 100. Range (0,inf)
			orderBy (str): the ordering type of the outputted peaks (ascending by frequency or descending by magnitude). Defaults to 'frequency'. Range {frequency,magnitude}
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
		...
		""" 
		... 
	def __call__(self, frame:list[Any], pitch:float) -> tuple[list[Any], list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			frame (list[Any]): the input frame. Defaults to . 
			pitch (float): external pitch input [Hz].. Defaults to . 
		Returns:
			frequencies (list[Any]): the frequencies of the sinusoidal peaks [Hz]
			magnitudes (list[Any]): the magnitudes of the sinusoidal peaks
			phases (list[Any]): the phases of the sinusoidal peaks
			res (list[Any]): output residual frame
		""" 
		... 
...
class HpsModelAnal(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any], pitch:float) -> tuple[list[Any], list[Any], list[Any], list[Any]]:
		...
		Returns:
			...
			magnitudes (list[Any]): the magnitudes of the sinusoidal peaks
			phases (list[Any]): the phases of the sinusoidal peaks
			stocenv (list[Any]): the stochastic envelope
		""" 
		... 
...
class HumDetector(_essentia.Algorithm): 
	def __init__(self, Q0:float=0.1, Q1:float=0.55, detectionThreshold:float=5, frameSize:float=0.4, hopSize:float=0.2, maximumFrequency:float=400, minimumDuration:float=0.5, minimumFrequency:float=27.5, numberHarmonics:int=1, sampleRate:float=44100, timeContinuity:float=10, timeWindow:float=9) -> None:
		...
		Args:
			...
			detectionThreshold (float): the detection threshold for the peaks of the r matrix. Defaults to 5. Range (0,inf)
			...
			maximumFrequency (float): maximum frequency to consider [Hz]. Defaults to 400. Range (0,inf)
			minimumDuration (float): minimun duration of the humming tones [s]. Defaults to 0.5. Range (0,inf)
			minimumFrequency (float): minimum frequency to consider [Hz]. Defaults to 27.5. Range (0,inf)
			numberHarmonics (int): number of considered harmonics. Defaults to 1. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			timeContinuity (float): time continuity cue (the maximum allowed gap duration for a pitch contour) [s]. Defaults to 10. Range (0,inf)
			timeWindow (float): analysis time to use for the hum estimation [s]. Defaults to 9. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> tuple[ndarray, list[Any], list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			r (ndarray): the quantile ratios matrix
			frequencies (list[Any]): humming tones frequencies
			saliences (list[Any]): humming tones saliences
			starts (list[Any]): humming tones starts
			ends (list[Any]): humming tones ends
		""" 
		... 
...
class IDCT(_essentia.Algorithm): 
	def __init__(self, dctType:int=2, inputSize:int=10, liftering:int=0, outputSize:int=10) -> None:
		...
		""" 
		... 
	def __call__(self, dct:list[Any]) -> list[Any]:
		"""compute
		Args:
			dct (list[Any]): the discrete cosine transform. Defaults to . 
		Returns:
			idct (list[Any]): the inverse cosine transform of the input array
		""" 
		... 
...
class IFFT(_essentia.Algorithm): 
	def __init__(self, normalize:bool=True, size:int=1024) -> None:
		...
		""" 
		... 
	def __call__(self, fft:ndarray) -> list[Any]:
		"""compute
		Args:
			fft (ndarray): the input frame. Defaults to . 
		Returns:
			frame (list[Any]): the IFFT of the input frame
		""" 
		... 
...
class IFFTC(_essentia.Algorithm): 
	def __init__(self, normalize:bool=True, size:int=1024) -> None:
		...
		""" 
		... 
	def __call__(self, fft:ndarray) -> ndarray:
		"""compute
		Args:
			fft (ndarray): the input frame. Defaults to . 
		Returns:
			frame (ndarray): the complex IFFT of the input frame
		""" 
		... 


class IIR(_essentia.Algorithm): 
	def __init__(self, denominator:list[Any]=[1], numerator:list[Any]=[1]) -> None:
		...
		Args:
			denominator (list[Any]): the list of coefficients of the denominator. Often referred to as the A coefficient vector.. Defaults to [1]. Range None
			numerator (list[Any]): the list of coefficients of the numerator. Often referred to as the B coefficient vector.. Defaults to [1]. Range None 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		"""compute
...
class Inharmonicity(_essentia.Algorithm): 
	...
	def __call__(self, frequencies:list[Any], magnitudes:list[Any]) -> float:
		"""compute
		Args:
			frequencies (list[Any]): the frequencies of the harmonic peaks [Hz] (in ascending order). Defaults to . 
			magnitudes (list[Any]): the magnitudes of the harmonic peaks (in frequency ascending order. Defaults to . 
		Returns:
...
class Key(_essentia.Algorithm): 
	def __init__(self, averageDetuningCorrection:bool=True, numHarmonics:int=4, pcpSize:int=36, pcpThreshold:float=0.2, profileType:str='bgate', slope:float=0.6, useMajMin:bool=False, usePolyphony:bool=True, useThreeChords:bool=True) -> None:
		...
		Args:
			...
			useMajMin (bool): songs can also be clasified as MajMin [4]. Only avalable for the edma, bgate and braw profiles. Defaults to False. Range {true,false}
		...
		""" 
		... 
	def __call__(self, pcp:list[Any]) -> tuple[str, str, float]:
		"""compute
		Args:
			pcp (list[Any]): the input pitch class profile. Defaults to . 
		Returns:
...
class KeyExtractor(_essentia.Algorithm): 
	def __init__(self, averageDetuningCorrection:bool=True, frameSize:int=4096, hopSize:int=4096, hpcpSize:int=12, maxFrequency:float=3500, maximumSpectralPeaks:int=60, minFrequency:float=25, pcpThreshold:float=0.2, profileType:str='bgate', sampleRate:float=44100, spectralPeaksThreshold:float=0.0001, tuningFrequency:float=440, weightType:str='cosine', windowType:str='hann') -> None:
		...
		Args:
			...
			maxFrequency (float): max frequency to apply whitening to [Hz]. Defaults to 3500. Range (0,inf)
			maximumSpectralPeaks (int): the maximum number of spectral peaks. Defaults to 60. Range (0,inf)
			minFrequency (float): min frequency to apply whitening to [Hz]. Defaults to 25. Range (0,inf)
			pcpThreshold (float): pcp bins below this value are set to 0. Defaults to 0.2. Range [0,1]
			...
			tuningFrequency (float): the tuning frequency of the input signal. Defaults to 440. Range (0,inf)
		...
		""" 
		... 
	def __call__(self, audio:float) -> tuple[str, str, float]:
		"""compute
		Args:
			audio (float): the audio signal. Defaults to . 
		Returns:
...
class LPC(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			lpc (list[Any]): the LPC coefficients
			reflection (list[Any]): the reflection coefficients
		""" 
		... 
...
class Larm(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> float:
		"""compute
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
...
class Leq(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): the input signal (must be non-empty). Defaults to . 
		Returns:
...
class LevelExtractor(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			loudness (float): the loudness values
		""" 
		... 
...
class LogAttackTime(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100, startAttackThreshold:float=0.2, stopAttackThreshold:float=0.9) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[float, float, float]:
		"""compute
		Args:
			signal (list[Any]): the input signal envelope (must be non-empty). Defaults to . 
		Returns:
...
class LogSpectrum(_essentia.Algorithm): 
	def __init__(self, binsPerSemitone:float=3, frameSize:int=1025, nOctave:int=7, rollOn:float=0, sampleRate:float=44100) -> None:
		...
		Args:
			binsPerSemitone (float):  bins per semitone. Defaults to 3. Range (0,inf)
			...
			rollOn (float): this removes low-frequency noise - useful in quiet recordings. Defaults to 0. Range [0,5]
			sampleRate (float): the input sample rate. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> tuple[list[Any], list[Any], float]:
		"""compute
		Args:
			spectrum (list[Any]): spectrum frame. Defaults to . 
		Returns:
			logFreqSpectrum (list[Any]): log frequency spectrum frame
			meanTuning (list[Any]): normalized mean tuning frequency
...
class LoopBpmConfidence(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any], bpmEstimate:float) -> float:
		"""compute
		Args:
			signal (list[Any]): loop audio signal. Defaults to . 
			bpmEstimate (float): estimated BPM for the audio signal (will be rounded to nearest integer). Defaults to . 
		Returns:
...
class LoudnessEBUR128(_essentia.Algorithm): 
	def __init__(self, hopSize:float=0.1, sampleRate:float=44100, startAtZero:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, signal:ndarray) -> tuple[float, float, float, float]:
		"""compute
		Args:
			signal (ndarray): the input stereo audio signal. Defaults to . 
		Returns:
			momentaryLoudness (float): momentary loudness (over 400ms) (LUFS)
			shortTermLoudness (float): short-term loudness (over 3 seconds) (LUFS)
...
class LoudnessEBUR128Filter(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100) -> None:
		"""Signal preprocessing algorithm used within the LoudnessEBUR128 algorithm.
		
		It applies the pre-processing K-weighting filter and computes signal representation requiered by LoudnessEBUR128 in accordance with the EBU R128 recommendation.
		...
		""" 
		... 
	def __call__(self, signal:ndarray) -> float:
		"""compute
		Args:
			signal (ndarray): the input stereo audio signal. Defaults to . 
		Returns:
			signal (float): the filtered signal (the sum of squared amplitudes of both channels filtered by ITU-R BS.1770 algorithm
		""" 
		... 
...
class LoudnessVickers(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100) -> None:
		...
		Args:
			sampleRate (float): the audio sampling rate of the input signal which is used to create the weight vector [Hz] (currently, this algorithm only works on signals with a sampling rate of 44100Hz). Defaults to 44100. Range [44100,44100] 
		""" 
		... 
	def __call__(self, signal:list[Any]) -> float:
		"""compute
...
class LowLevelSpectralEqloudExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, hopSize:int=1024, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, float, list[Any], list[Any], float, float, float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			spectral_centroid (float): See Centroid algorithm documentation
			dissonance (float): See Dissonance algorithm documentation
			sccoeffs (list[Any]): See SpectralContrast algorithm documentation
			scvalleys (list[Any]): See SpectralContrast algorithm documentation
			spectral_kurtosis (float): See DistributionShape algorithm documentation
			spectral_skewness (float): See DistributionShape algorithm documentation
			spectral_spread (float): See DistributionShape algorithm documentation
		""" 
		... 
...
class LowLevelSpectralExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, hopSize:int=1024, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[list[Any], float, float, float, float, list[Any], float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, list[Any], float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			...
			barkbands_skewness (float): skewness from bark bands. See DistributionShape algorithm documentation
			barkbands_spread (float): spread from barkbands. See DistributionShape algorithm documentation
			hfc (float): See HFC algorithm documentation
			mfcc (list[Any]): See MFCC algorithm documentation
			pitch (float): See PitchYinFFT algorithm documentation
			pitch_instantaneous_confidence (float): See PitchYinFFT algorithm documentation
			pitch_salience (float): See PitchSalience algorithm documentation
			silence_rate_20dB (float): See SilenceRate algorithm documentation
			silence_rate_30dB (float): See SilenceRate algorithm documentation
			silence_rate_60dB (float): See SilenceRate algorithm documentation
			spectral_complexity (float): See Spectral algorithm documentation
			spectral_crest (float): See Crest algorithm documentation
			spectral_decrease (float): See Decrease algorithm documentation
			spectral_energy (float): See Energy algorithm documentation
			spectral_energyband_low (float): Energy in band (20,150] Hz. See EnergyBand algorithm documentation
			spectral_energyband_middle_low (float): Energy in band (150,800] Hz.See EnergyBand algorithm documentation
			spectral_energyband_middle_high (float): Energy in band (800,4000] Hz. See EnergyBand algorithm documentation
			spectral_energyband_high (float): Energy in band (4000,20000] Hz. See EnergyBand algorithm documentation
			spectral_flatness_db (float): See flatnessDB algorithm documentation
			spectral_flux (float): See Flux algorithm documentation
			spectral_rms (float): See RMS algorithm documentation
			spectral_rolloff (float): See RollOff algorithm documentation
			spectral_strongpeak (float): See StrongPeak algorithm documentation
			zerocrossingrate (float): See ZeroCrossingRate algorithm documentation
			inharmonicity (float): See Inharmonicity algorithm documentation
			tristimulus (list[Any]): See Tristimulus algorithm documentation
			oddtoevenharmonicenergyratio (float): See OddToEvenHarmonicEnergyRatio algorithm documentation
		""" 
		... 
...
class MFCC(_essentia.Algorithm): 
	...
	def __call__(self, spectrum:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Args:
			spectrum (list[Any]): the audio spectrum. Defaults to . 
		Returns:
			bands (list[Any]): the energies in mel bands
			mfcc (list[Any]): the mel frequency cepstrum coefficients
		""" 
		... 
...
class Magnitude(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, complex:ndarray) -> list[Any]:
		"""compute
		Args:
			complex (ndarray): the input vector of complex numbers. Defaults to . 
		Returns:
			magnitude (list[Any]): the magnitudes of the input vector
		""" 
		... 
...
class MaxFilter(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): signal to be filtered. Defaults to . 
		Returns:
			signal (float): filtered output
		""" 
		... 
...
class MaxMagFreq(_essentia.Algorithm): 
	...
	def __call__(self, spectrum:list[Any]) -> float:
		"""compute
		Args:
			spectrum (list[Any]): the input spectrum (must have more than 1 element). Defaults to . 
		Returns:
...
class MaxToTotal(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, envelope:float) -> float:
		"""compute
		Args:
			envelope (float): the envelope of the signal. Defaults to . 
		Returns:
...
class Median(_essentia.Algorithm): 
	...
	def __call__(self, array:list[Any]) -> float:
		"""compute
		Args:
			array (list[Any]): the input array (must be non-empty). Defaults to . 
		Returns:
...
class MedianFilter(_essentia.Algorithm): 
	...
	def __call__(self, array:list[Any]) -> list[Any]:
		...
		Args:
			array (list[Any]): the input array (must be non-empty). Defaults to . 
		Returns:
			filteredArray (list[Any]): the median-filtered input array
		""" 
		... 
...
class MelBands(_essentia.Algorithm): 
	def __init__(self, highFrequencyBound:float=22050, inputSize:int=1025, log:bool=False, lowFrequencyBound:float=0, normalize:str='unit_sum', numberBands:int=24, sampleRate:float=44100, type:str='power', warpingFormula:str='htkMel', weighting:str='warping') -> None:
		...
		Args:
			...
			lowFrequencyBound (float): a lower-bound limit for the frequencies to be included in the bands. Defaults to 0. Range [0,inf)
			normalize (str): spectrum bin weights to use for each mel band: 'unit_max' to make each mel band vertex equal to 1, 'unit_sum' to make each mel band area equal to 1 summing the actual weights of spectrum bins, 'unit_area' to make each triangle mel band area equal to 1 normalizing the weights of each triangle by its bandwidth. Defaults to 'unit_sum'. Range {unit_sum,unit_tri,unit_max}
			numberBands (int): the number of output bands. Defaults to 24. Range (1,inf)
			sampleRate (float): the sample rate. Defaults to 44100. Range (0,inf)
	...
	def __call__(self, spectrum:list[Any]) -> list[Any]:
		...
		Args:
			spectrum (list[Any]): the audio spectrum. Defaults to . 
		Returns:
			bands (list[Any]): the energy in mel bands
		""" 
		... 
...
class MetadataReader(_essentia.Algorithm): 
	def __init__(self, failOnError:bool=False, filename:str="") -> None:
		...
		Args:
			...
			filename (str): the name of the file from which to read the tags. Defaults to . Range None 
		""" 
		... 
	def __call__(self, ) -> tuple[str, str, str, str, str, str, str, int, int, int, int]:
		"""compute
		Args:
...
class Meter(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, beatogram:ndarray) -> float:
		"""compute
		Args:
			beatogram (ndarray): filtered matrix loudness. Defaults to . 
		Returns:
...
class MinMax(_essentia.Algorithm): 
	def __init__(self, type:str='min') -> None:
		...
		""" 
		... 
	def __call__(self, array:list[Any]) -> tuple[float, int]:
		"""compute
		Args:
			array (list[Any]): the input array. Defaults to . 
		Returns:
...
class MonoLoader(_essentia.Algorithm): 
	def __init__(self, audioStream:int=0, downmix:str='mix', filename:str='None', resampleQuality:int=1, sampleRate:float=44100) -> None:
		...
		Args:
			...
			sampleRate (float): the desired output sampling rate [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, ) -> float:
		"""compute
		...
		Returns:
			audio (float): the mono audio signal
		""" 
		... 
...
class MonoMixer(_essentia.Algorithm): 
	def __init__(self, type:str='mix') -> None:
		...
		""" 
		... 
	def __call__(self, audio:ndarray, numberChannels:int) -> float:
		"""compute
		Args:
			audio (ndarray): the input stereo signal. Defaults to . 
			numberChannels (int): the number of channels of the input signal. Defaults to . 
		Returns:
			audio (float): the downmixed signal
		""" 
		... 
...
class MonoWriter(_essentia.Algorithm): 
	def __init__(self, bitrate:int=192, filename:str='None', format:str='wav', sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, audio:float) -> None:
		"""compute
		Args:
			audio (float): the input audio. Defaults to . 
		Returns:
None
		""" 
		... 
...
class MultiPitchMelodia(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, filterIterations:int=3, frameSize:int=2048, guessUnvoiced:bool=False, harmonicWeight:float=0.8, hopSize:int=128, magnitudeCompression:float=1, magnitudeThreshold:int=40, maxFrequency:float=20000, minDuration:int=100, minFrequency:float=80, numberHarmonics:int=20, peakDistributionThreshold:float=0.9, peakFrameThreshold:float=0.9, pitchContinuity:float=27.5625, referenceFrequency:float=55, sampleRate:float=44100, timeContinuity:int=100) -> None:
		...
		Args:
			binResolution (float): salience function bin resolution [cents]. Defaults to 10. Range (0,inf)
			...
			guessUnvoiced (bool): guess pitch using non-salient contours when no salient ones are present in a frame. Defaults to False. Range {false,true}
			...
			magnitudeCompression (float): magnitude compression parameter (=0 for maximum compression, =1 for no compression). Defaults to 1. Range (0,1]
			magnitudeThreshold (int): peak magnitude threshold (maximum allowed difference from the highest peak in dBs). Defaults to 40. Range [0,inf)
			maxFrequency (float): the maximum allowed frequency for salience function peaks (ignore peaks above) [Hz]. Defaults to 20000. Range [0,inf)
			minDuration (int): the minimum allowed contour duration [ms]. Defaults to 100. Range (0,inf)
			minFrequency (float): the minimum allowed frequency for salience function peaks (ignore peaks below) [Hz]. Defaults to 80. Range [0,inf)
			numberHarmonics (int): number of considered hamonics. Defaults to 20. Range [1,inf)
			peakDistributionThreshold (float): allowed deviation below the peak salience mean over all frames (fraction of the standard deviation). Defaults to 0.9. Range [0,1]
			...
			referenceFrequency (float): the reference frequency for Hertz to cent convertion [Hz], corresponding to the 0th cent bin. Defaults to 55. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			timeContinuity (int): tine continuity cue (the maximum allowed gap duration for a pitch contour) [ms]. Defaults to 100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> ndarray:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			pitch (ndarray): the estimated pitch values per frames [Hz]
		""" 
		... 
...
class Multiplexer(_essentia.Algorithm): 
	...
	def __call__(self, ) -> list[Any]:
		...
		Returns:
			data (list[Any]): the frame containing the input values and/or input frames
		""" 
		... 
...
class NNLSChroma(_essentia.Algorithm): 
	def __init__(self, chromaNormalization:str='none', frameSize:int=1025, sampleRate:float=44100, spectralShape:float=0.7, spectralWhitening:float=1, tuningMode:str='global', useNNLS:bool=True) -> None:
		...
		Args:
			...
			spectralWhitening (float): determines how much the log-frequency spectrum is whitened. Defaults to 1. Range [0,1.0]
		...
		""" 
		... 
	def __call__(self, logSpectrogram:ndarray, meanTuning:list[Any], localTuning:list[Any]) -> tuple[ndarray, ndarray, ndarray, ndarray]:
		"""compute
		Args:
			logSpectrogram (ndarray): log spectrum frames. Defaults to . 
			meanTuning (list[Any]): mean tuning frames. Defaults to . 
			localTuning (list[Any]): local tuning frames. Defaults to . 
		Returns:
			tunedLogfreqSpectrum (ndarray): Log frequency spectrum after tuning
			semitoneSpectrum (ndarray): a spectral representation with one bin per semitone
			bassChromagram (ndarray):  a 12-dimensional chromagram, restricted to the bass range
			chromagram (ndarray): a 12-dimensional chromagram, restricted with mid-range emphasis
		""" 
		... 
...
class NSGConstantQ(_essentia.Algorithm): 
	def __init__(self, binsPerOctave:int=48, gamma:int=0, inputSize:int=4096, maxFrequency:float=7040, minFrequency:float=27.5, minimumWindow:int=4, normalize:str='none', phaseMode:str='global', rasterize:str='full', sampleRate:float=44100, window:str='hannnsgcq', windowSizeFactor:int=1) -> None:
		...
		Args:
			...
			maxFrequency (float): the maximum frequency. Defaults to 7040. Range (0,inf)
			...
			sampleRate (float): the desired sampling rate [Hz]. Defaults to 44100. Range [0,inf)
		...
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[ndarray, ndarray, ndarray]:
		"""compute
		Args:
			frame (list[Any]): the input frame (vector). Defaults to . 
		Returns:
			constantq (ndarray): the constant Q transform of the input frame
			constantqdc (ndarray): the DC band transform of the input frame. Only needed for the inverse transform
			constantqnf (ndarray): the Nyquist band transform of the input frame. Only needed for the inverse transform
		""" 
		... 


class NSGConstantQStreaming(_essentia.Algorithm): 
	def __init__(self, binsPerOctave:int=48, gamma:int=0, inputSize:int=4096, maxFrequency:float=7040, minFrequency:float=27.5, minimumWindow:int=4, normalize:str='none', phaseMode:str='global', rasterize:str='full', sampleRate:float=44100, window:str='hannnsgcq', windowSizeFactor:int=1) -> None:
		"""Computes a constant Q transform using non stationary Gabor frames and returns a complex time-frequency representation of the input vector.
		...
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[ndarray, ndarray, ndarray, int]:
		"""compute
		Args:
			frame (list[Any]): the input audio signal. Defaults to . 
		Returns:
			constantq (ndarray): the constant Q transform of the input frame
			constantqdc (ndarray): the DC band transform of the input frame. Only needed for the inverse transform
			constantqnf (ndarray): the Nyquist band transform of the input frame. Only needed for the inverse transform
			framestamps (int): this vector sets the beginnings of each frame in the 'constantq' buffer
		""" 
		... 
...
class NSGIConstantQ(_essentia.Algorithm): 
	def __init__(self, binsPerOctave:int=48, gamma:int=0, inputSize:int=4096, maxFrequency:float=7040, minFrequency:float=27.5, minimumWindow:int=4, normalize:str='none', phaseMode:str='global', rasterize:str='full', sampleRate:float=44100, window:str='hannnsgcq', windowSizeFactor:int=1) -> None:
		...
		""" 
		... 
	def __call__(self, constantq:ndarray, constantqdc:ndarray, constantqnf:ndarray) -> list[Any]:
		"""compute
		Args:
			constantq (ndarray): the constant Q transform of the input frame. Defaults to . 
			constantqdc (ndarray): the DC band transform of the input frame. Defaults to . 
			constantqnf (ndarray): the Nyquist band transform of the input frame. Defaults to . 
		Returns:
			frame (list[Any]): the input frame (vector)
		""" 
		... 
...
class NoiseBurstDetector(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> list[Any]:
		...
		Args:
			frame (list[Any]): the input frame (must be non-empty). Defaults to . 
		Returns:
			indexes (list[Any]): indexes of the noisy samples
		""" 
		... 


class NoveltyCurve(_essentia.Algorithm): 
	def __init__(self, frameRate:float=344.531, normalize:bool=False, weightCurve:list[Any]=[], weightCurveType:str='inverse_quadratic') -> None:
		...
		Args:
			...
			weightCurve (list[Any]): vector containing the weights for each frequency band. Only if weightCurveType==supplied. Defaults to []. Range None
			weightCurveType (str): the type of weighting to be used for the bands novelty. Defaults to 'inverse_quadratic'. Range {flat,triangle,inverse_triangle,parabola,inverse_parabola,linear,quadratic,inverse_quadratic,supplied} 
		""" 
		... 
	def __call__(self, frequencyBands:list[Any]) -> float:
		"""compute
		Args:
			frequencyBands (list[Any]): the frequency bands. Defaults to . 
		Returns:
			novelty (float): the novelty curve as a single vector
		""" 
		... 
...
class OddToEvenHarmonicEnergyRatio(_essentia.Algorithm): 
	...
	def __call__(self, frequencies:list[Any], magnitudes:list[Any]) -> float:
		"""compute
		Args:
			frequencies (list[Any]): the frequencies of the harmonic peaks (at least two frequencies in frequency ascending order). Defaults to . 
			magnitudes (list[Any]): the magnitudes of the harmonic peaks (at least two magnitudes in frequency ascending order). Defaults to . 
		Returns:
...
class OnsetDetection(_essentia.Algorithm): 
	def __init__(self, method:str='hfc', sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, spectrum:list[Any], phase:list[Any]) -> float:
		"""compute
		Args:
			spectrum (list[Any]): the input spectrum. Defaults to . 
			phase (list[Any]): the phase vector corresponding to this spectrum (used only by the "complex" method). Defaults to . 
		Returns:
...
class OnsetDetectionGlobal(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			onsetDetections (float): the frame-wise values of the detection function
		""" 
		... 
...
class OnsetRate(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[list[Any], float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			onsetTimes (list[Any]): the detected onset times [s]
...
class Onsets(_essentia.Algorithm): 
	def __init__(self, alpha:float=0.1, delay:int=5, frameRate:float=86.1328, silenceThreshold:float=0.02) -> None:
		...
		""" 
		... 
	def __call__(self, detections:ndarray, weights:list[Any]) -> list[Any]:
		"""compute
		Args:
			detections (ndarray): matrix containing onset detection functions--rows represent the values of different detection functions and columns represent different frames of audio (i.e. detections[i][j] represents the value of the ith detection function for the jth frame of audio). Defaults to . 
			weights (list[Any]): the weighting coefficicients for each detection function, must be the same as the first dimension of "detections". Defaults to . 
		Returns:
			onsets (list[Any]): the onset positions [s]
		""" 
		... 
...
class OverlapAdd(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, gain:float=1, hopSize:int=128) -> None:
		...
		Args:
			frameSize (int): the frame size for computing the overlap-add process. Defaults to 2048. Range (0,inf)
			gain (float): the normalization gain that scales the output signal. Useful for IFFT output. Defaults to 1. Range (0.,inf)
			hopSize (int): the hop size with which the overlap-add function is computed. Defaults to 128. Range (0,inf) 
		""" 
		... 
	def __call__(self, frame:list[Any]) -> float:
		"""compute
		Args:
			frame (list[Any]): the windowed input audio frame. Defaults to . 
		Returns:
			signal (float): the output overlap-add audio signal
		""" 
		... 
...
class Panning(_essentia.Algorithm): 
	def __init__(self, averageFrames:int=43, numBands:int=1, numCoeffs:int=20, panningBins:int=512, sampleRate:float=44100, warpedPanorama:bool=True) -> None:
		...
		Args:
			...
			sampleRate (float): audio sampling rate [Hz]. Defaults to 44100. Range (0,inf)
		...
		""" 
		... 
	def __call__(self, spectrumLeft:list[Any], spectrumRight:list[Any]) -> ndarray:
		"""compute
		Args:
			spectrumLeft (list[Any]): left channel's spectrum. Defaults to . 
			spectrumRight (list[Any]): right channel's spectrum. Defaults to . 
		Returns:
			panningCoeffs (ndarray): parameters that define the panning curve at each frame
		""" 
		... 
...
class PeakDetection(_essentia.Algorithm): 
	def __init__(self, interpolate:bool=True, maxPeaks:int=100, maxPosition:float=1, minPeakDistance:float=0, minPosition:float=0, orderBy:str='position', range:float=1, threshold:float=-1e+06) -> None:
		...
		Args:
			...
			maxPeaks (int): the maximum number of returned peaks. Defaults to 100. Range [1,inf)
			maxPosition (float): the maximum value of the range to evaluate. Defaults to 1. Range (0,inf)
			minPeakDistance (float): minimum distance between consecutive peaks (0 to bypass this feature). Defaults to 0. Range [0,inf)
			minPosition (float): the minimum value of the range to evaluate. Defaults to 0. Range [0,inf)
			...
			range (float): the input range. Defaults to 1. Range (0,inf)
			threshold (float): peaks below this given threshold are not output. Defaults to -1e+06. Range (-inf,inf) 
		""" 
		... 
	def __call__(self, array:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			array (list[Any]): the input array. Defaults to . 
		Returns:
			positions (list[Any]): the positions of the peaks
			amplitudes (list[Any]): the amplitudes of the peaks
		""" 
		... 
...
class PercivalEnhanceHarmonics(_essentia.Algorithm): 
	...
	def __call__(self, array:list[Any]) -> list[Any]:
		"""compute
		Args:
			array (list[Any]): the input signal. Defaults to . 
		Returns:
			array (list[Any]): the input signal with enhanced harmonics
		""" 
		... 
...
class PercivalEvaluatePulseTrains(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, oss:list[Any], positions:list[Any]) -> float:
		"""compute
		Args:
			oss (list[Any]): onset strength signal (or other novelty curve). Defaults to . 
			positions (list[Any]): peak positions of BPM candidates. Defaults to . 
		Returns:
...
class PitchContours(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, hopSize:int=128, minDuration:float=100, peakDistributionThreshold:float=0.9, peakFrameThreshold:float=0.9, pitchContinuity:float=27.5625, sampleRate:float=44100, timeContinuity:float=100) -> None:
		...
		Args:
			binResolution (float): salience function bin resolution [cents]. Defaults to 10. Range (0,inf)
			hopSize (int): the hop size with which the pitch salience function was computed. Defaults to 128. Range (0,inf)
			minDuration (float): the minimum allowed contour duration [ms]. Defaults to 100. Range (0,inf)
			...
			pitchContinuity (float): pitch continuity cue (maximum allowed pitch change durig 1 ms time period) [cents]. Defaults to 27.5625. Range [0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			timeContinuity (float): time continuity cue (the maximum allowed gap duration for a pitch contour) [ms]. Defaults to 100. Range (0,inf) 
		""" 
		... 
	def __call__(self, peakBins:ndarray, peakSaliences:ndarray) -> tuple[ndarray, ndarray, list[Any], float]:
		"""compute
		Args:
			peakBins (ndarray): frame-wise array of cent bins corresponding to pitch salience function peaks. Defaults to . 
			peakSaliences (ndarray): frame-wise array of values of salience function peaks. Defaults to . 
		Returns:
			contoursBins (ndarray): array of frame-wise vectors of cent bin values representing each contour
			contoursSaliences (ndarray): array of frame-wise vectors of pitch saliences representing each contour
			contoursStartTimes (list[Any]): array of start times of each contour [s]
...
class PitchContoursMelody(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, filterIterations:int=3, guessUnvoiced:bool=False, hopSize:int=128, maxFrequency:float=20000, minFrequency:float=80, referenceFrequency:float=55, sampleRate:float=44100, voiceVibrato:bool=False, voicingTolerance:float=0.2) -> None:
		...
		Args:
			...
			hopSize (int): the hop size with which the pitch salience function was computed. Defaults to 128. Range (0,inf)
			maxFrequency (float): the maximum allowed frequency for salience function peaks (ignore contours with peaks above) [Hz]. Defaults to 20000. Range [0,inf)
			minFrequency (float): the minimum allowed frequency for salience function peaks (ignore contours with peaks below) [Hz]. Defaults to 80. Range [0,inf)
			referenceFrequency (float): the reference frequency for Hertz to cent convertion [Hz], corresponding to the 0th cent bin. Defaults to 55. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal (Hz). Defaults to 44100. Range (0,inf)
		...
		""" 
		... 
	def __call__(self, contoursBins:ndarray, contoursSaliences:ndarray, contoursStartTimes:list[Any], duration:float) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			contoursBins (ndarray): array of frame-wise vectors of cent bin values representing each contour. Defaults to . 
			contoursSaliences (ndarray): array of frame-wise vectors of pitch saliences representing each contour. Defaults to . 
			contoursStartTimes (list[Any]): array of the start times of each contour [s]. Defaults to . 
			duration (float): time duration of the input signal [s]. Defaults to . 
		Returns:
			pitch (list[Any]): vector of estimated pitch values (i.e., melody) [Hz]
			pitchConfidence (list[Any]): confidence with which the pitch was detected
		""" 
		... 
...
class PitchContoursMultiMelody(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, filterIterations:int=3, guessUnvoiced:bool=False, hopSize:int=128, maxFrequency:float=20000, minFrequency:float=80, referenceFrequency:float=55, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, contoursBins:ndarray, contoursSaliences:ndarray, contoursStartTimes:list[Any], duration:float) -> ndarray:
		"""compute
		Args:
			contoursBins (ndarray): array of frame-wise vectors of cent bin values representing each contour. Defaults to . 
			contoursSaliences (ndarray): array of frame-wise vectors of pitch saliences representing each contour. Defaults to . 
			...
			duration (float): time duration of the input signal [s]. Defaults to . 
		Returns:
			pitch (ndarray): vector of estimated pitch values (i.e., melody) [Hz]
		""" 
		... 
...
class PitchFilter(_essentia.Algorithm): 
	def __init__(self, confidenceThreshold:int=36, minChunkSize:int=30, useAbsolutePitchConfidence:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, pitchConfidence:list[Any], pitch:list[Any]) -> list[Any]:
		"""compute
		Args:
			pitchConfidence (list[Any]): vector of pitch confidence values for the input frames. Defaults to . 
			pitch (list[Any]): vector of pitch values for the input frames [Hz]. Defaults to . 
		Returns:
			pitchFiltered (list[Any]): vector of corrected pitch values [Hz]
		""" 
		... 
...
class PitchMelodia(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, filterIterations:int=3, frameSize:int=2048, guessUnvoiced:bool=False, harmonicWeight:float=0.8, hopSize:int=128, magnitudeCompression:float=1, magnitudeThreshold:int=40, maxFrequency:float=20000, minDuration:int=100, minFrequency:float=80, numberHarmonics:int=20, peakDistributionThreshold:float=0.9, peakFrameThreshold:float=0.9, pitchContinuity:float=27.5625, referenceFrequency:float=55, sampleRate:float=44100, timeContinuity:int=100) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			pitch (list[Any]): the estimated pitch values per frames [Hz]
			pitchConfidence (list[Any]): the estimated pitch confidence
		""" 
		... 
...
class PitchSalience(_essentia.Algorithm): 
	def __init__(self, highBoundary:float=5000, lowBoundary:float=100, sampleRate:float=44100) -> None:
		...
		Args:
			highBoundary (float): until which frequency we are looking for the minimum (must be smaller than half sampleRate) [Hz]. Defaults to 5000. Range (0,inf)
			lowBoundary (float): from which frequency we are looking for the maximum (must not be larger than highBoundary) [Hz]. Defaults to 100. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> float:
...
class PitchSalienceFunction(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, harmonicWeight:float=0.8, magnitudeCompression:float=1, magnitudeThreshold:float=40, numberHarmonics:int=20, referenceFrequency:float=55) -> None:
		...
		Args:
			binResolution (float): salience function bin resolution [cents]. Defaults to 10. Range (0,100]
			harmonicWeight (float): harmonic weighting parameter (weight decay ratio between two consequent harmonics, =1 for no decay). Defaults to 0.8. Range [0,1]
			magnitudeCompression (float): magnitude compression parameter (=0 for maximum compression, =1 for no compression). Defaults to 1. Range (0,1]
			magnitudeThreshold (float): peak magnitude threshold (maximum allowed difference from the highest peak in dBs). Defaults to 40. Range [0,inf)
	...
	def __call__(self, frequencies:list[Any], magnitudes:list[Any]) -> list[Any]:
		...
		Args:
			...
			magnitudes (list[Any]): the magnitudes of the spectral peaks. Defaults to . 
		Returns:
			salienceFunction (list[Any]): array of the quantized pitch salience values
		""" 
		... 
...
class PitchSalienceFunctionPeaks(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, maxFrequency:float=1760, minFrequency:float=55, referenceFrequency:float=55) -> None:
		...
		Args:
			binResolution (float): salience function bin resolution [cents]. Defaults to 10. Range (0,inf)
			maxFrequency (float): the maximum frequency to evaluate (ignore peaks above) [Hz]. Defaults to 1760. Range [0,inf)
			minFrequency (float): the minimum frequency to evaluate (ignore peaks below) [Hz]. Defaults to 55. Range [0,inf)
			referenceFrequency (float): the reference frequency for Hertz to cent convertion [Hz], corresponding to the 0th cent bin. Defaults to 55. Range (0,inf) 
		""" 
		... 
	def __call__(self, salienceFunction:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			salienceFunction (list[Any]): the array of salience function values corresponding to cent frequency bins. Defaults to . 
		Returns:
			salienceBins (list[Any]): the cent bins corresponding to salience function peaks
			salienceValues (list[Any]): the values of salience function peaks
		""" 
		... 
...
class PitchYin(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, interpolate:bool=True, maxFrequency:float=22050, minFrequency:float=20, sampleRate:float=44100, tolerance:float=0.15) -> None:
		...
		Args:
			...
			maxFrequency (float): the maximum allowed frequency [Hz]. Defaults to 22050. Range (0,inf)
			minFrequency (float): the minimum allowed frequency [Hz]. Defaults to 20. Range (0,inf)
			sampleRate (float): sampling rate of the input audio [Hz]. Defaults to 44100. Range (0,inf)
			tolerance (float): tolerance for peak detection. Defaults to 0.15. Range [0,1] 
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[float, float]:
		"""compute
		Args:
			signal (list[Any]): the input signal frame. Defaults to . 
		Returns:
...
class PitchYinFFT(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, interpolate:bool=True, maxFrequency:float=22050, minFrequency:float=20, sampleRate:float=44100, tolerance:float=1, weighting:str='custom') -> None:
		...
		Args:
			...
			maxFrequency (float): the maximum allowed frequency [Hz]. Defaults to 22050. Range (0,inf)
			minFrequency (float): the minimum allowed frequency [Hz]. Defaults to 20. Range (0,inf)
			sampleRate (float): sampling rate of the input spectrum [Hz]. Defaults to 44100. Range (0,inf)
			tolerance (float): tolerance for peak detection. Defaults to 1. Range [0,1]
			weighting (str): string to assign a weighting function. Defaults to 'custom'. Range {custom,A,B,C,D,Z} 
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> tuple[float, float]:
		"""compute
		Args:
			spectrum (list[Any]): the input spectrum (preferably created with a hann window). Defaults to . 
		Returns:
			pitch (float): detected pitch [Hz]
			pitchConfidence (float): confidence with which the pitch was detected [0,1]
		""" 
...
class PitchYinProbabilistic(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			signal (float): the input mono audio signal. Defaults to . 
		Returns:
			pitch (list[Any]): the output pitch estimations
			voicedProbabilities (list[Any]): the voiced probabilities
		""" 
		... 
...
class PitchYinProbabilities(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, lowAmp:float=0.1, preciseTime:bool=False, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[list[Any], list[Any], float]:
		"""compute
		Args:
			signal (list[Any]): the input signal frame. Defaults to . 
		Returns:
			pitch (list[Any]): the output pitch candidate frequencies in cents
			probabilities (list[Any]): the output pitch candidate probabilities
...
class PitchYinProbabilitiesHMM(_essentia.Algorithm): 
	def __init__(self, minFrequency:float=61.735, numberBinsPerSemitone:int=5, selfTransition:float=0.99, yinTrust:float=0.5) -> None:
		...
		""" 
		... 
	def __call__(self, pitchCandidates:ndarray, probabilities:ndarray) -> list[Any]:
		"""compute
		Args:
			pitchCandidates (ndarray): the pitch candidates. Defaults to . 
			probabilities (ndarray): the pitch probabilities. Defaults to . 
		Returns:
			pitch (list[Any]): pitch frequencies in Hz
		""" 
		... 
...
class PolarToCartesian(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, magnitude:list[Any], phase:list[Any]) -> ndarray:
		"""compute
		Args:
			magnitude (list[Any]): the magnitude vector. Defaults to . 
			phase (list[Any]): the phase vector. Defaults to . 
		Returns:
			complex (ndarray): the resulting complex vector
		""" 
		... 


class PoolAggregator(_essentia.Algorithm): 
	def __init__(self, defaultStats:str='["mean", "stdev", "min", "max", "median"]', exceptions:dict[Any, Any]={}) -> None:
		...
		Args:
			defaultStats (str): the default statistics to be computed for each descriptor in the input pool. Defaults to '["mean", "stdev", "min", "max", "median"]'. Range None
	...
	def __call__(self, input:Pool) -> Pool:
		"""compute
		Args:
			input (Pool): the input pool. Defaults to . 
		Returns:
...
class PoolToTensor(_essentia.Algorithm): 
	def __init__(self, namespace:str='input_0') -> None:
		"""Retrieve a tensor from a pool under a given namespace 

		Args:
			namespace (str): the input namespace. Defaults to 'input_0'. Range None 
		""" 
		... 
	def __call__(self, pool:Pool) -> ndarray:
		"""compute
		Args:
			pool (Pool): the pool containing the required namespace. Defaults to . 
		Returns:
			tensor (ndarray): tensors retrieved from the pool
		""" 
		... 
...
class PowerMean(_essentia.Algorithm): 
	def __init__(self, power:float=1) -> None:
		...
		Args:
			power (float): the power to which to elevate each element before taking the mean. Defaults to 1. Range (-inf,inf) 
		""" 
		... 
	def __call__(self, array:list[Any]) -> float:
		"""compute
		Args:
			array (list[Any]): the input array (must contain only positive real numbers). Defaults to . 
		Returns:
...
class PowerSpectrum(_essentia.Algorithm): 
	def __init__(self, size:int=2048) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> list[Any]:
		"""compute
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			powerSpectrum (list[Any]): power spectrum of the input signal
		""" 
		... 
...
class RawMoments(_essentia.Algorithm): 
	def __init__(self, range:float=22050) -> None:
		...
		Args:
			range (float): the range of the input array, used for normalizing the results. Defaults to 22050. Range (0,inf) 
		""" 
		... 
	def __call__(self, array:list[Any]) -> list[Any]:
		"""compute
		Args:
			array (list[Any]): the input array. Defaults to . 
		Returns:
			rawMoments (list[Any]): the (raw) moments of the input array
		""" 
		... 


class RealAccumulator(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, data:float) -> list[Any]:
		"""compute
		Args:
			data (float): the input signal. Defaults to . 
		Returns:
			array (list[Any]): the accumulated signal in one single frame
		""" 
		... 
...
class ReplayGain(_essentia.Algorithm): 
	def __init__(self, applyEqloud:bool=True, sampleRate:float=44100) -> None:
		...
		Args:
			applyEqloud (bool): enables whether this algorithm should apply an equal-loudness filter (set to false if the input audio signal is already equal-loudness filtered). Defaults to True. Range {true,false}
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			replayGain (float): the ReplayGain gain value in dB
		""" 
		... 
...
class Resample(_essentia.Algorithm): 
	def __init__(self, inputSampleRate:float=44100, outputSampleRate:float=44100, quality:int=1) -> None:
		...
		Args:
			inputSampleRate (float): the sampling rate of the input signal [Hz]. Defaults to 44100. Range (0,inf)
			outputSampleRate (float): the sampling rate of the output signal [Hz]. Defaults to 44100. Range (0,inf)
	...
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			signal (float): the resampled signal
		""" 
		... 
...
class ResampleFFT(_essentia.Algorithm): 
	def __init__(self, inSize:int=128, outSize:int=128) -> None:
		...
		""" 
		... 
	def __call__(self, input:list[Any]) -> list[Any]:
		"""compute
		Args:
			input (list[Any]): input array. Defaults to . 
		Returns:
			output (list[Any]): output resample array
		""" 
		... 
...
class RhythmDescriptors(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[list[Any], float, float, list[Any], list[Any], float, float, float, float, float, float, list[Any]]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			beats_position (list[Any]): See RhythmExtractor2013 algorithm documentation
			...
			bpm_estimates (list[Any]): See RhythmExtractor2013 algorithm documentation
			bpm_intervals (list[Any]): See RhythmExtractor2013 algorithm documentation
...
class RhythmExtractor(_essentia.Algorithm): 
	def __init__(self, frameHop:int=1024, frameSize:int=1024, hopSize:int=256, lastBeatInterval:float=0.1, maxTempo:int=208, minTempo:int=40, numberFrames:int=1024, sampleRate:float=44100, tempoHints:list[Any]=[], tolerance:float=0.24, useBands:bool=True, useOnset:bool=True) -> None:
		...
		Args:
			...
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			tempoHints (list[Any]): the optional list of initial beat locations, to favor the detection of pre-determined tempo period and beats alignment [s]. Defaults to []. Range None
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			signal (float): input signal. Defaults to . 
		Returns:
			bpm (float): the tempo estimation [bpm]
			ticks (list[Any]):  the estimated tick locations [s]
			estimates (list[Any]): the list of bpm estimates characterizing the bpm distribution for the signal [bpm]
			bpmIntervals (list[Any]): list of beats interval [s]
		""" 
		... 
...
class RhythmExtractor2013(_essentia.Algorithm): 
	def __init__(self, maxTempo:int=208, method:str='multifeature', minTempo:int=40) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[list[Any], float, float, list[Any], list[Any]]:
		"""compute
		Args:
			signal (float): input signal. Defaults to . 
		Returns:
...
class RhythmTransform(_essentia.Algorithm): 
	def __init__(self, frameSize:int=256, hopSize:int=32) -> None:
		...
		""" 
		... 
	def __call__(self, melBands:list[Any]) -> ndarray:
		"""compute
		Args:
			melBands (list[Any]): the energy in the melbands. Defaults to . 
		Returns:
			rhythm (ndarray): consecutive frames in the rhythm domain
		""" 
		... 
...
class RollOff(_essentia.Algorithm): 
	def __init__(self, cutoff:float=0.85, sampleRate:float=44100) -> None:
		...
		Args:
			...
			sampleRate (float): the sampling rate of the audio signal (used to normalize rollOff) [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, spectrum:list[Any]) -> float:
		"""compute
		Args:
			spectrum (list[Any]): the input audio spectrum (must have more than one elements). Defaults to . 
		Returns:
...
class SBic(_essentia.Algorithm): 
	def __init__(self, cpw:float=1.5, inc1:int=60, inc2:int=20, minLength:int=10, size1:int=300, size2:int=200) -> None:
		...
		""" 
		... 
	def __call__(self, features:ndarray) -> list[Any]:
		"""compute
		Args:
			features (ndarray): extracted features matrix (rows represent features, and columns represent frames of audio). Defaults to . 
		Returns:
			segmentation (list[Any]): a list of frame indices that indicate where a segment of audio begins/ends (the indices of the first and last frame are also added to the list at the beginning and end, respectively)
		""" 
		... 
...
class SNR(_essentia.Algorithm): 
	def __init__(self, MAAlpha:float=0.95, MMSEAlpha:float=0.98, NoiseAlpha:float=0.9, frameSize:int=512, noiseThreshold:float=-40, sampleRate:float=44100, useBroadbadNoiseCorrection:bool=True) -> None:
		...
		Args:
			...
			noiseThreshold (float): Threshold to detect frames without signal. Defaults to -40. Range (-inf,0]
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
		...
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[float, float, list[Any]]:
		"""compute
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			...
			spectralSNR (list[Any]): instant SNR for each frequency bin
		""" 
		... 
...
class SaturationDetector(_essentia.Algorithm): 
	def __init__(self, differentialThreshold:float=0.001, energyThreshold:float=-1, frameSize:int=512, hopSize:int=256, minimumDuration:float=0.005, sampleRate:float=44100) -> None:
		...
		Args:
			...
			energyThreshold (float): mininimum energy of the samples in the saturated regions [dB]. Defaults to -1. Range (-inf,0]
	...
	def __call__(self, frame:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			starts (list[Any]): starting times of the detected saturated regions [s]
			ends (list[Any]): ending times of the detected saturated regions [s]
		""" 
		... 
...
class Scale(_essentia.Algorithm): 
	def __init__(self, clipping:bool=True, factor:float=10, maxAbsValue:float=1) -> None:
		...
		Args:
			...
			factor (float): the multiplication factor by which the audio will be scaled. Defaults to 10. Range [0,inf)
			maxAbsValue (float): the maximum value above which to apply clipping. Defaults to 1. Range [0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			signal (float): the output audio signal
		""" 
		... 


class SilenceRate(_essentia.Algorithm): 
	def __init__(self, thresholds:list[Any]=[]) -> None:
		...
		Args:
			thresholds (list[Any]): the threshold values. Defaults to []. Range None 
		""" 
		... 
	def __call__(self, frame:list[Any]) -> None:
		"""compute
		Args:
			frame (list[Any]): the input frame. Defaults to . 
		Returns:
...
class SineModelAnal(_essentia.Algorithm): 
	def __init__(self, freqDevOffset:float=20, freqDevSlope:float=0.01, magnitudeThreshold:float=-74, maxFrequency:float=22050, maxPeaks:int=250, maxnSines:int=100, minFrequency:float=0, orderBy:str='frequency', sampleRate:float=44100) -> None:
		...
		Args:
			...
			freqDevSlope (float): slope increase of minimum frequency deviation. Defaults to 0.01. Range (-inf,inf)
			magnitudeThreshold (float): peaks below this given threshold are not outputted. Defaults to -74. Range (-inf,inf)
			maxFrequency (float): the maximum frequency of the range to evaluate [Hz]. Defaults to 22050. Range (0,inf)
			maxPeaks (int): the maximum number of returned peaks. Defaults to 250. Range [1,inf)
			maxnSines (int): maximum number of sines per frame. Defaults to 100. Range (0,inf)
			minFrequency (float): the minimum frequency of the range to evaluate [Hz]. Defaults to 0. Range [0,inf)
			orderBy (str): the ordering type of the outputted peaks (ascending by frequency or descending by magnitude). Defaults to 'frequency'. Range {frequency,magnitude}
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, fft:ndarray) -> tuple[list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			fft (ndarray): the input frame. Defaults to . 
		Returns:
...
class SineModelSynth(_essentia.Algorithm): 
	def __init__(self, fftSize:int=2048, hopSize:int=512, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, magnitudes:list[Any], frequencies:list[Any], phases:list[Any]) -> ndarray:
		"""compute
		Args:
			magnitudes (list[Any]): the magnitudes of the sinusoidal peaks. Defaults to . 
			frequencies (list[Any]): the frequencies of the sinusoidal peaks [Hz]. Defaults to . 
			phases (list[Any]): the phases of the sinusoidal peaks. Defaults to . 
		Returns:
			fft (ndarray): the output FFT frame
		""" 
		... 
...
class SineSubtraction(_essentia.Algorithm): 
	def __init__(self, fftSize:int=512, hopSize:int=128, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, frame:list[Any], magnitudes:list[Any], frequencies:list[Any], phases:list[Any]) -> list[Any]:
		"""compute
		Args:
			frame (list[Any]): the input audio frame to subtract from. Defaults to . 
			magnitudes (list[Any]): the magnitudes of the sinusoidal peaks. Defaults to . 
			frequencies (list[Any]): the frequencies of the sinusoidal peaks [Hz]. Defaults to . 
			phases (list[Any]): the phases of the sinusoidal peaks. Defaults to . 
		Returns:
			frame (list[Any]): the output audio frame
		""" 
		... 


class SingleBeatLoudness(_essentia.Algorithm): 
	def __init__(self, beatDuration:float=0.05, beatWindowDuration:float=0.1, frequencyBands:list[Any]=[0, 200, 400, 800, 1600, 3200, 22000], onsetStart:str='sumEnergy', sampleRate:float=44100) -> None:
		...
		Args:
			...
			frequencyBands (list[Any]): frequency bands. Defaults to [0, 200, 400, 800, 1600, 3200, 22000]. Range None
			onsetStart (str): criteria for finding the start of the beat. Defaults to 'sumEnergy'. Range {sumEnergy,peakEnergy}
			sampleRate (float): the audio sampling rate [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, beat:list[Any]) -> tuple[float, list[Any]]:
		"""compute
		Args:
			beat (list[Any]): audio segement containing a beat. Defaults to . 
		Returns:
			loudness (float): the beat's energy across the whole spectrum
			loudnessBandRatio (list[Any]): the beat's energy ratio for each band
		""" 
		... 
...
class SingleGaussian(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, matrix:ndarray) -> tuple[list[Any], ndarray, ndarray]:
		"""compute
		Args:
			matrix (ndarray): the input data matrix (e.g. the MFCC descriptor over frames). Defaults to . 
		Returns:
			mean (list[Any]): the mean of the values
			covariance (ndarray): the covariance matrix
			inverseCovariance (ndarray): the inverse of the covariance matrix
		""" 
		... 


class Slicer(_essentia.Algorithm): 
	def __init__(self, endTimes:list[Any]=[], sampleRate:float=44100, startTimes:list[Any]=[], timeUnits:str='seconds') -> None:
		...
		Args:
			endTimes (list[Any]): the list of end times for the slices you want to extract. Defaults to []. Range None
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			startTimes (list[Any]): the list of start times for the slices you want to extract. Defaults to []. Range None
			timeUnits (str): the units of time of the start and end times. Defaults to 'seconds'. Range {samples,seconds} 
		""" 
		... 
	def __call__(self, audio:float) -> list[Any]:
		"""compute
		Args:
			audio (float): the input signal. Defaults to . 
		Returns:
			frame (list[Any]): the frames of the sliced input signal
		""" 
		... 
...
class SpectralContrast(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, highFrequencyBound:float=11000, lowFrequencyBound:float=20, neighbourRatio:float=0.4, numberBands:int=6, sampleRate:float=22050, staticDistribution:float=0.15) -> None:
		...
		Args:
			...
			highFrequencyBound (float): the upper bound of the highest band. Defaults to 11000. Range (0,inf)
			lowFrequencyBound (float): the lower bound of the lowest band. Defaults to 20. Range (0,inf)
			...
			sampleRate (float): the sampling rate of the audio signal. Defaults to 22050. Range (0,inf)
	...
	def __call__(self, spectrum:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Args:
			spectrum (list[Any]): the audio spectrum. Defaults to . 
		Returns:
			spectralContrast (list[Any]): the spectral contrast coefficients
			spectralValley (list[Any]): the magnitudes of the valleys
		""" 
		... 
...
class SpectralPeaks(_essentia.Algorithm): 
	...
	def __call__(self, spectrum:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Args:
			spectrum (list[Any]): the input spectrum. Defaults to . 
		Returns:
			frequencies (list[Any]): the frequencies of the spectral peaks [Hz]
			magnitudes (list[Any]): the magnitudes of the spectral peaks
		""" 
		... 
...
class SpectralWhitening(_essentia.Algorithm): 
	def __init__(self, maxFrequency:float=5000, sampleRate:float=44100) -> None:
		...
		Args:
			maxFrequency (float): max frequency to apply whitening to [Hz]. Defaults to 5000. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, spectrum:list[Any], frequencies:list[Any], magnitudes:list[Any]) -> list[Any]:
		"""compute
		Args:
			spectrum (list[Any]): the audio linear spectrum. Defaults to . 
			frequencies (list[Any]): the spectral peaks' linear frequencies. Defaults to . 
			magnitudes (list[Any]): the spectral peaks' linear magnitudes. Defaults to . 
		Returns:
			magnitudes (list[Any]): the whitened spectral peaks' linear magnitudes
		""" 
		... 
...
class Spectrum(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> list[Any]:
		...
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			spectrum (list[Any]): magnitude spectrum of the input audio signal
		""" 
		... 
...
class SpectrumCQ(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> list[Any]:
		...
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			spectrumCQ (list[Any]): the magnitude constant-Q spectrum
		""" 
		... 
...
class SpectrumToCent(_essentia.Algorithm): 
	def __init__(self, bands:int=720, centBinResolution:float=10, inputSize:int=32768, log:bool=True, minimumFrequency:float=164, normalize:str='unit_sum', sampleRate:float=44100, type:str='power') -> None:
		...
		Args:
			...
			centBinResolution (float): Width of each band in cents. Default is 10 cents. Defaults to 10. Range (0,inf)
			...
			minimumFrequency (float): central frequency of the first band of the bank [Hz]. Defaults to 164. Range (0,inf)
	...
	def __call__(self, spectrum:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Returns:
			bands (list[Any]): the energy in each band
			frequencies (list[Any]): the central frequency of each band
		""" 
		... 


class Spline(_essentia.Algorithm): 
	def __init__(self, beta1:float=1, beta2:float=0, type:str='b', xPoints:list[Any]=[0, 1], yPoints:list[Any]=[0, 1]) -> None:
		...
		Args:
			beta1 (float): the skew or bias parameter (only available for type beta). Defaults to 1. Range [0,inf]
			beta2 (float): the tension parameter. Defaults to 0. Range [0,inf)
...
class SprModelAnal(_essentia.Algorithm): 
	def __init__(self, fftSize:int=2048, freqDevOffset:int=20, freqDevSlope:float=0.01, hopSize:int=512, magnitudeThreshold:float=0, maxFrequency:float=5000, maxPeaks:int=100, maxnSines:int=100, minFrequency:float=0, orderBy:str='frequency', sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[list[Any], list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			frame (list[Any]): the input frame. Defaults to . 
		Returns:
...
class SprModelSynth(_essentia.Algorithm): 
	def __init__(self, fftSize:int=2048, hopSize:int=512, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, magnitudes:list[Any], frequencies:list[Any], phases:list[Any], res:list[Any]) -> tuple[list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			magnitudes (list[Any]): the magnitudes of the sinusoidal peaks. Defaults to . 
			frequencies (list[Any]): the frequencies of the sinusoidal peaks [Hz]. Defaults to . 
			phases (list[Any]): the phases of the sinusoidal peaks. Defaults to . 
			res (list[Any]): the residual frame. Defaults to . 
		Returns:
			frame (list[Any]): the output audio frame of the Sinusoidal Plus Stochastic model
			sineframe (list[Any]): the output audio frame for sinusoidal component 
			resframe (list[Any]): the output audio frame for stochastic component 
		""" 
		... 
...
class SpsModelSynth(_essentia.Algorithm): 
	def __init__(self, fftSize:int=2048, hopSize:int=512, sampleRate:float=44100, stocf:float=0.2) -> None:
		...
		""" 
		... 
	def __call__(self, magnitudes:list[Any], frequencies:list[Any], phases:list[Any], stocenv:list[Any]) -> tuple[list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			magnitudes (list[Any]): the magnitudes of the sinusoidal peaks. Defaults to . 
			frequencies (list[Any]): the frequencies of the sinusoidal peaks [Hz]. Defaults to . 
			phases (list[Any]): the phases of the sinusoidal peaks. Defaults to . 
			stocenv (list[Any]): the stochastic envelope. Defaults to . 
		Returns:
			frame (list[Any]): the output audio frame of the Sinusoidal Plus Stochastic model
			sineframe (list[Any]): the output audio frame for sinusoidal component 
			stocframe (list[Any]): the output audio frame for stochastic component 
		""" 
		... 
...
class StartStopCut(_essentia.Algorithm): 
	def __init__(self, frameSize:int=256, hopSize:int=256, maximumStartTime:float=10, maximumStopTime:float=10, sampleRate:float=44100, threshold:int=-60) -> None:
		...
		Args:
			...
			maximumStartTime (float): if the first non-silent frame occurs before maximumStartTime startCut is activated [ms]. Defaults to 10. Range [0,inf)
			maximumStopTime (float): if the last non-silent frame occurs after maximumStopTime to the end stopCut is activated [ms]. Defaults to 10. Range [0,inf)
			sampleRate (float): the sample rate. Defaults to 44100. Range (0,inf)
			threshold (int): the threshold below which average energy is defined as silence [dB]. Defaults to -60. Range (-inf,0] 
		""" 
		... 
	def __call__(self, audio:list[Any]) -> tuple[int, int]:
		"""compute
		Args:
			audio (list[Any]): the input audio . Defaults to . 
		Returns:
...
class StartStopSilence(_essentia.Algorithm): 
	def __init__(self, threshold:int=-60) -> None:
		...
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[int, int]:
		"""compute
		Args:
			frame (list[Any]): the input audio frames. Defaults to . 
		Returns:
...
class StereoDemuxer(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, audio:ndarray) -> tuple[float, float]:
		"""compute
		Args:
			audio (ndarray): the input stereo signal. Defaults to . 
		Returns:
			left (float): the left channel of the audio signal
			right (float): the right channel of the audio signal
		""" 
		... 
...
class StereoMuxer(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, left:float, right:float) -> ndarray:
		"""compute
		Args:
			left (float): the left channel of the audio signal. Defaults to . 
			right (float): the right channel of the audio signal. Defaults to . 
		Returns:
			audio (ndarray): the output stereo signal
		""" 
		... 
...
class StereoTrimmer(_essentia.Algorithm): 
	def __init__(self, endTime:float=1e+06, sampleRate:float=44100, startTime:float=0) -> None:
		...
		Args:
			endTime (float): the end time of the slice you want to extract [s]. Defaults to 1e+06. Range [0,inf)
			sampleRate (float): the sampling rate of the input audio signal [Hz]. Defaults to 44100. Range (0,inf)
			startTime (float): the start time of the slice you want to extract [s]. Defaults to 0. Range [0,inf) 
		""" 
		... 
	def __call__(self, signal:ndarray) -> ndarray:
		"""compute
		Args:
			signal (ndarray): the input stereo signal. Defaults to . 
		Returns:
			signal (ndarray): the trimmed stereo signal
		""" 
		... 
...
class StochasticModelSynth(_essentia.Algorithm): 
	def __init__(self, fftSize:int=2048, hopSize:int=512, sampleRate:float=44100, stocf:float=0.2) -> None:
		...
		""" 
		... 
	def __call__(self, stocenv:list[Any]) -> list[Any]:
		"""compute
		Args:
			stocenv (list[Any]): the stochastic envelope input. Defaults to . 
		Returns:
			frame (list[Any]): the output frame
		""" 
		... 
...
class StrongPeak(_essentia.Algorithm): 
	...
	def __call__(self, spectrum:list[Any]) -> float:
		"""compute
		Args:
			spectrum (list[Any]): the input spectrum (must be greater than one element and cannot contain negative values). Defaults to . 
		Returns:
...
class SuperFluxExtractor(_essentia.Algorithm): 
	def __init__(self, combine:float=20, frameSize:int=2048, hopSize:int=256, ratioThreshold:float=16, sampleRate:float=44100, threshold:float=0.05) -> None:
		...
		Args:
			combine (float): time threshold for double onsets detections (ms). Defaults to 20. Range (0,inf)
			frameSize (int): the frame size for computing low-level features. Defaults to 2048. Range (0,inf)
			hopSize (int): the hop size for computing low-level features. Defaults to 256. Range (0,inf)
			ratioThreshold (float): ratio threshold for peak picking with respect to novelty_signal/novelty_average rate, use 0 to disable it (for low-energy onsets). Defaults to 16. Range [0,inf)
			sampleRate (float): the audio sampling rate [Hz]. Defaults to 44100. Range (0,inf)
	...
	def __call__(self, signal:float) -> list[Any]:
		...
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			onsets (list[Any]): lists of onsets
		""" 
		... 
...
class SuperFluxNovelty(_essentia.Algorithm): 
	def __init__(self, binWidth:int=3, frameWidth:int=2) -> None:
		...
		""" 
		... 
	def __call__(self, bands:list[Any]) -> float:
		"""compute
		Args:
			bands (list[Any]): the input bands spectrogram. Defaults to . 
		Returns:
...
class SuperFluxPeaks(_essentia.Algorithm): 
	def __init__(self, combine:float=30, frameRate:float=172, pre_avg:float=100, pre_max:float=30, ratioThreshold:float=16, threshold:float=0.05) -> None:
		...
		Args:
			combine (float): ms for onset combination. Defaults to 30. Range (0,inf)
			frameRate (float): frameRate. Defaults to 172. Range (0,inf)
			pre_avg (float): look back duration for moving average filter [ms]. Defaults to 100. Range (0,inf)
			pre_max (float): look back duration for moving maximum filter [ms]. Defaults to 30. Range (0,inf)
			ratioThreshold (float): ratio threshold for peak picking with respect to novelty_signal/novelty_average rate, use 0 to disable it (for low-energy onsets). Defaults to 16. Range [0,inf)
			threshold (float): threshold for peak peaking with respect to the difference between novelty_signal and average_signal (for onsets in ambient noise). Defaults to 0.05. Range [0,inf) 
		""" 
		... 
	def __call__(self, novelty:float) -> list[Any]:
		"""compute
		Args:
			novelty (float): the input novelty. Defaults to . 
		Returns:
			peaks (list[Any]): peaks instants [s]
		""" 
		... 
...
class TCToTotal(_essentia.Algorithm): 
	...
	def __call__(self, envelope:float) -> float:
		"""compute
		Args:
			envelope (float): the envelope of the signal (its length must be greater than 1. Defaults to . 
		Returns:
...
class TempoScaleBands(_essentia.Algorithm): 
	def __init__(self, bandsGain:list[Any]=[2, 3, 2, 1, 1.20000004768, 2, 3, 2.5], frameTime:float=512) -> None:
		...
		Args:
			bandsGain (list[Any]): gain for each bands. Defaults to [2, 3, 2, 1, 1.20000004768, 2, 3, 2.5]. Range None
			frameTime (float): the frame rate in samples. Defaults to 512. Range (0,inf) 
		""" 
		... 
	def __call__(self, bands:list[Any]) -> tuple[list[Any], float]:
		"""compute
		Args:
			bands (list[Any]): the audio power spectrum divided into bands. Defaults to . 
		Returns:
			scaledBands (list[Any]): the output bands after scaling
...
class TempoTap(_essentia.Algorithm): 
	def __init__(self, frameHop:int=1024, frameSize:int=256, maxTempo:int=208, minTempo:int=40, numberFrames:int=1024, sampleRate:float=44100, tempoHints:list[Any]=[]) -> None:
		...
		Args:
			...
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			tempoHints (list[Any]): optional list of initial beat locations, to favor the detection of pre-determined tempo period and beats alignment [s]. Defaults to []. Range None 
		""" 
		... 
	def __call__(self, featuresFrame:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			featuresFrame (list[Any]): input temporal features of a frame. Defaults to . 
		Returns:
			periods (list[Any]): list of tempo estimates found for each input feature, in frames
			phases (list[Any]): list of initial phase candidates found for each input feature, in frames
		""" 
		... 
...
class TempoTapDegara(_essentia.Algorithm): 
	def __init__(self, maxTempo:int=208, minTempo:int=40, resample:str='none', sampleRateODF:float=86.1328) -> None:
		...
		""" 
		... 
	def __call__(self, onsetDetections:float) -> float:
		"""compute
		Args:
			onsetDetections (float): per-frame onset detection values. Defaults to . 
		Returns:
			ticks (float): the list of resulting ticks [s]
		""" 
		... 
...
class TempoTapMaxAgreement(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, tickCandidates:ndarray) -> tuple[list[Any], float]:
		"""compute
		Args:
			tickCandidates (ndarray): the tick candidates estimated using different beat trackers (or features) [s]. Defaults to . 
		Returns:
			ticks (list[Any]): the list of resulting ticks [s]
...
class TempoTapTicks(_essentia.Algorithm): 
	def __init__(self, frameHop:int=512, hopSize:int=256, sampleRate:float=44100) -> None:
		...
		Args:
			...
			sampleRate (float): sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, periods:list[Any], phases:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			periods (list[Any]): tempo period candidates for the current frame, in frames. Defaults to . 
			phases (list[Any]): tempo ticks phase candidates for the current frame, in frames. Defaults to . 
		Returns:
			ticks (list[Any]): the list of resulting ticks [s]
			matchingPeriods (list[Any]): list of matching periods [s]
		""" 
		... 
...
class TensorNormalize(_essentia.Algorithm): 
	...
	def __call__(self, tensor:ndarray) -> ndarray:
		"""compute
		Args:
			tensor (ndarray): the input tensor. Defaults to . 
		Returns:
			tensor (ndarray): the normalized output tensor
		""" 
		... 


class TensorToPool(_essentia.Algorithm): 
	def __init__(self, mode:str='overwrite', namespace:str='input_0') -> None:
		"""Inserts a tensor into a pool under a given namespace.
		
		Suppors adding (accumulation) or (setting) overwritting 

		Args:
			mode (str): what to do with the input tensors. Defaults to 'overwrite'. Range {add,overwrite}
			namespace (str): where to add the input tensor. Defaults to 'input_0'. Range None 
		""" 
		... 
	def __call__(self, tensor:ndarray) -> Pool:
		"""compute
		Args:
			tensor (ndarray): the tensor to be added to the pool. Defaults to . 
		Returns:
...
class TensorToVectorReal(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, tensor:ndarray) -> list[Any]:
		"""compute
		Args:
			tensor (ndarray): the input tensor. Defaults to . 
		Returns:
			frame (list[Any]): the frames to be retrieved from the tensor
		""" 
		... 


class TensorTranspose(_essentia.Algorithm): 
	def __init__(self, permutation:ndarray|None=None) -> None:
		"""Performs transpositions over the axes of a tensor.
		
		 

		Args:
			permutation (ndarray|None): permutation of [0,1,2,3]. The i'th dimension of the returned tensor will correspond to the dimension numbered permutation[i] of the input.. Defaults to None. Range None 
		""" 
		... 
	def __call__(self, tensor:ndarray) -> ndarray:
		"""compute
		Args:
			tensor (ndarray): the input tensor. Defaults to . 
		Returns:
			tensor (ndarray): the transposed output tensor
		""" 
		... 
...
class TensorflowInputFSDSINet(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> list[Any]:
		"""compute
		Args:
			frame (list[Any]): the audio frame. Defaults to . 
		Returns:
			bands (list[Any]): the log-compressed mel bands
		""" 
		... 
...
class TonalExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=4096, hopSize:int=2048, tuningFrequency:float=440) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, list[Any], str, float, str, str, float, list[Any], list[Any], str, str, float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			chords_changes_rate (float): See ChordsDescriptors algorithm documentation
			chords_histogram (list[Any]): See ChordsDescriptors algorithm documentation
			...
			chords_progression (str): See ChordsDetection algorithm documentation
			chords_scale (str): See ChordsDetection algorithm documentation
			chords_strength (float): See ChordsDetection algorithm documentation
			hpcp (list[Any]): See HPCP algorithm documentation
			hpcp_highres (list[Any]): See HPCP algorithm documentation
...
class TriangularBands(_essentia.Algorithm): 
	def __init__(self, frequencyBands:list[Any]=[21.533203125, 43.06640625, 64.599609375, 86.1328125, 107.666015625, 129.19921875, 150.732421875, 172.265625, 193.798828125, 215.33203125, 236.865234375, 258.3984375, 279.931640625, 301.46484375, 322.998046875, 344.53125, 366.064453125, 387.59765625, 409.130859375, 430.6640625, 452.197265625, 473.73046875, 495.263671875, 516.796875, 538.330078125, 559.86328125, 581.396484375, 602.9296875, 624.462890625, 645.99609375, 667.529296875, 689.0625, 710.595703125, 732.12890625, 753.662109375, 775.1953125, 796.728515625, 839.794921875, 861.328125, 882.861328125, 904.39453125, 925.927734375, 968.994140625, 990.52734375, 1012.06054688, 1055.12695312, 1076.66015625, 1098.19335938, 1141.25976562, 1184.32617188, 1205.859375, 1248.92578125, 1270.45898438, 1313.52539062, 1356.59179688, 1399.65820312, 1442.72460938, 1485.79101562, 1528.85742188, 1571.92382812, 1614.99023438, 1658.05664062, 1701.12304688, 1765.72265625, 1808.7890625, 1873.38867188, 1916.45507812, 1981.0546875, 2024.12109375, 2088.72070312, 2153.3203125, 2217.91992188, 2282.51953125, 2347.11914062, 2411.71875, 2497.8515625, 2562.45117188, 2627.05078125, 2713.18359375, 2799.31640625, 2885.44921875, 2950.04882812, 3036.18164062, 3143.84765625, 3229.98046875, 3316.11328125, 3423.77929688, 3509.91210938, 3617.578125, 3725.24414062, 3832.91015625, 3940.57617188, 4069.77539062, 4177.44140625, 4306.640625, 4435.83984375, 4565.0390625, 4694.23828125, 4844.97070312, 4974.16992188, 5124.90234375, 5275.63476562, 5426.3671875, 5577.09960938, 5749.36523438, 5921.63085938, 6093.89648438, 6266.16210938, 6459.9609375, 6653.75976562, 6847.55859375, 7041.35742188, 7256.68945312, 7450.48828125, 7687.35351562, 7902.68554688, 8139.55078125, 8376.41601562, 8613.28125, 8871.6796875, 9130.078125, 9388.4765625, 9668.40820312, 9948.33984375, 10249.8046875, 10551.2695312, 10852.734375, 11175.7324219, 11498.7304688, 11843.2617188, 12187.7929688, 12553.8574219, 12919.921875, 13285.9863281, 13673.5839844, 14082.7148438, 14491.8457031, 14922.5097656, 15353.1738281, 15805.3710938, 16257.5683594], inputSize:int=1025, log:bool=True, normalize:str='unit_sum', sampleRate:float=44100, type:str='power', weighting:str='linear') -> None:
		...
		Args:
			frequencyBands (list[Any]): list of frequency ranges into which the spectrum is divided (these must be in ascending order and connot contain duplicates),each triangle is build as x(i-1)=0, x(i)=1, x(i+1)=0 over i, the resulting number of bands is size of input array - 2. Defaults to [21.533203125, 43.06640625, 64.599609375, 86.1328125, 107.666015625, 129.19921875, 150.732421875, 172.265625, 193.798828125, 215.33203125, 236.865234375, 258.3984375, 279.931640625, 301.46484375, 322.998046875, 344.53125, 366.064453125, 387.59765625, 409.130859375, 430.6640625, 452.197265625, 473.73046875, 495.263671875, 516.796875, 538.330078125, 559.86328125, 581.396484375, 602.9296875, 624.462890625, 645.99609375, 667.529296875, 689.0625, 710.595703125, 732.12890625, 753.662109375, 775.1953125, 796.728515625, 839.794921875, 861.328125, 882.861328125, 904.39453125, 925.927734375, 968.994140625, 990.52734375, 1012.06054688, 1055.12695312, 1076.66015625, 1098.19335938, 1141.25976562, 1184.32617188, 1205.859375, 1248.92578125, 1270.45898438, 1313.52539062, 1356.59179688, 1399.65820312, 1442.72460938, 1485.79101562, 1528.85742188, 1571.92382812, 1614.99023438, 1658.05664062, 1701.12304688, 1765.72265625, 1808.7890625, 1873.38867188, 1916.45507812, 1981.0546875, 2024.12109375, 2088.72070312, 2153.3203125, 2217.91992188, 2282.51953125, 2347.11914062, 2411.71875, 2497.8515625, 2562.45117188, 2627.05078125, 2713.18359375, 2799.31640625, 2885.44921875, 2950.04882812, 3036.18164062, 3143.84765625, 3229.98046875, 3316.11328125, 3423.77929688, 3509.91210938, 3617.578125, 3725.24414062, 3832.91015625, 3940.57617188, 4069.77539062, 4177.44140625, 4306.640625, 4435.83984375, 4565.0390625, 4694.23828125, 4844.97070312, 4974.16992188, 5124.90234375, 5275.63476562, 5426.3671875, 5577.09960938, 5749.36523438, 5921.63085938, 6093.89648438, 6266.16210938, 6459.9609375, 6653.75976562, 6847.55859375, 7041.35742188, 7256.68945312, 7450.48828125, 7687.35351562, 7902.68554688, 8139.55078125, 8376.41601562, 8613.28125, 8871.6796875, 9130.078125, 9388.4765625, 9668.40820312, 9948.33984375, 10249.8046875, 10551.2695312, 10852.734375, 11175.7324219, 11498.7304688, 11843.2617188, 12187.7929688, 12553.8574219, 12919.921875, 13285.9863281, 13673.5839844, 14082.7148438, 14491.8457031, 14922.5097656, 15353.1738281, 15805.3710938, 16257.5683594]. Range None
			inputSize (int): the size of the spectrum. Defaults to 1025. Range (1,inf)
			log (bool): compute log-energies (log2 (1 + energy)). Defaults to True. Range {true,false}
...
class TriangularBarkBands(_essentia.Algorithm): 
	...
	def __call__(self, spectrum:list[Any]) -> list[Any]:
		...
		Args:
			spectrum (list[Any]): the audio spectrum. Defaults to . 
		Returns:
			bands (list[Any]): the energy in bark bands
		""" 
		... 
...
class Trimmer(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input signal. Defaults to . 
		Returns:
			signal (float): the trimmed signal
		""" 
		... 
...
class Tristimulus(_essentia.Algorithm): 
	...
	def __call__(self, frequencies:list[Any], magnitudes:list[Any]) -> list[Any]:
		"""compute
		Args:
			frequencies (list[Any]): the frequencies of the harmonic peaks ordered by frequency. Defaults to . 
			magnitudes (list[Any]): the magnitudes of the harmonic peaks ordered by frequency. Defaults to . 
		Returns:
			tristimulus (list[Any]): a three-element vector that measures the mixture of harmonics of the given spectrum
		""" 
		... 
...
class TruePeakDetector(_essentia.Algorithm): 
	def __init__(self, blockDC:bool=False, emphasise:bool=False, oversamplingFactor:int=4, quality:int=1, sampleRate:float=44100, threshold:float=-0.0002, version:int=4) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the input audio signal. Defaults to . 
		Returns:
			output (list[Any]): the processed signal
			peakLocations (list[Any]): the peak locations in the ouput signal
		""" 
		... 
...
class TuningFrequency(_essentia.Algorithm): 
	def __init__(self, resolution:float=1) -> None:
		...
		Args:
			resolution (float): resolution in cents (logarithmic scale, 100 cents = 1 semitone) for tuning frequency determination. Defaults to 1. Range (0,inf) 
		""" 
		... 
	def __call__(self, frequencies:list[Any], magnitudes:list[Any]) -> tuple[float, float]:
		"""compute
		Args:
			frequencies (list[Any]): the frequencies of the spectral peaks [Hz]. Defaults to . 
			magnitudes (list[Any]): the magnitudes of the spectral peaks. Defaults to . 
...
class TuningFrequencyExtractor(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		...
		Args:
			signal (float): the input audio signal. Defaults to . 
		Returns:
			tuningFrequency (float): the computed tuning frequency
		""" 
		... 
...
class UnaryOperator(_essentia.Algorithm): 
	def __init__(self, scale:float=1, shift:float=0, type:str='identity') -> None:
		...
		Args:
			scale (float): multiply result by factor. Defaults to 1. Range (-inf,inf)
			shift (float): shift result by value (add value). Defaults to 0. Range (-inf,inf)
	...
	def __call__(self, array:list[Any]) -> list[Any]:
		...
		Args:
			array (list[Any]): the input array. Defaults to . 
		Returns:
			array (list[Any]): the input array transformed by unary operation
		""" 
		... 
...
class UnaryOperatorStream(_essentia.Algorithm): 
	def __init__(self, scale:float=1, shift:float=0, type:str='identity') -> None:
		...
		""" 
		... 
	def __call__(self, array:float) -> float:
		"""compute
		Args:
			array (float): the input array. Defaults to . 
		Returns:
			array (float): the input array transformed by unary operation
		""" 
		... 
...
class VectorInput(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		"""Used as the starting point of a streaming network.
...
class VectorRealAccumulator(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		"""Takes a stream of Real values and outputs them as a single vector when the end of the stream is reached. 
		...
		""" 
		... 
	def __call__(self, data:list[Any]) -> ndarray:
		"""compute
		Args:
			data (list[Any]): the input signal. Defaults to . 
		Returns:
			array (ndarray): the accumulated signal in one single frame
		""" 
		... 


class VectorRealToTensor(_essentia.Algorithm): 
	def __init__(self, batchHopSize:int=0, lastBatchMode:str='push', lastPatchMode:str='repeat', patchHopSize:int=0, shape:list=[1, 1, 187, 96]) -> None:
		"""Generates tensors out of a stream of input frames.
		
		The 4 dimensions of the tensors stand for (batchSize, channels, patchSize, featureSize):
		...
		Args:
			batchHopSize (int): number of patches between the beginnings of adjacent batches. Use `0` to avoid overlap. Defaults to 0. Range [0,inf)
			lastBatchMode (str): what to do with the last patches: `push` an incomplete batch (if the models accepts dynamic batches) or `discard` them. Defaults to 'push'. Range {discard,push}
			lastPatchMode (str): what to do with the last frames: `repeat` them to fill the last patch or `discard` them. Defaults to 'repeat'. Range {discard,repeat}
			patchHopSize (int): number of frames between the beginnings of adjacent patches. Use `0` to avoid overlap. Defaults to 0. Range [0,inf)
			shape (ndarray): shape of the output tensor (batchSize, channels, patchSize, featureSize). If batchSize is set to -1 or 0 a single tensor is generated when the end of the stream is reached. Defaults to [1, 1, 187, 96]. Range None 
		""" 
		... 
	def __call__(self, frame:list[Any]) -> ndarray:
		"""compute
		Args:
			frame (list[Any]): the input frames. Defaults to . 
		Returns:
			tensor (ndarray): the accumulated frame in one single tensor
		""" 
		... 
...
class Vibrato(_essentia.Algorithm): 
	def __init__(self, maxExtend:float=250, maxFrequency:float=8, minExtend:float=50, minFrequency:float=4, sampleRate:float=344.531) -> None:
		...
		Args:
			maxExtend (float): maximum considered vibrato extent [cents]. Defaults to 250. Range (0,inf)
			maxFrequency (float): maximum considered vibrato frequency [Hz]. Defaults to 8. Range (0,inf)
			minExtend (float): minimum considered vibrato extent [cents]. Defaults to 50. Range (0,inf)
			minFrequency (float): minimum considered vibrato frequency [Hz]. Defaults to 4. Range (0,inf)
			sampleRate (float): sample rate of the input pitch contour. Defaults to 344.531. Range (0,inf) 
		""" 
		... 
	def __call__(self, pitch:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			pitch (list[Any]): the pitch trajectory [Hz].. Defaults to . 
		Returns:
			vibratoFrequency (list[Any]): estimated vibrato frequency (or speed) [Hz]; zero if no vibrato was detected.
			vibratoExtend (list[Any]): estimated vibrato extent (or depth) [cents]; zero if no vibrato was detected.
		""" 
		... 
...
class Viterbi(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, observationProbabilities:ndarray, initialization:list[Any], fromIndex:ndarray, toIndex:ndarray, transitionProbabilities:list[Any]) -> ndarray:
		"""compute
		Args:
			observationProbabilities (ndarray): the observation probabilities. Defaults to . 
			initialization (list[Any]): the initialization. Defaults to . 
			fromIndex (ndarray): the transition matrix from index. Defaults to . 
			toIndex (ndarray): the transition matrix to index. Defaults to . 
			transitionProbabilities (list[Any]): the transition probabilities matrix. Defaults to . 
		Returns:
			path (ndarray): the decoded path
		""" 
		... 
...
class WarpedAutoCorrelation(_essentia.Algorithm): 
	...
	def __call__(self, array:list[Any]) -> list[Any]:
		...
		Args:
			array (list[Any]): the array to be analyzed. Defaults to . 
		Returns:
			warpedAutoCorrelation (list[Any]): the warped auto-correlation vector
		""" 
		... 
...
class Welch(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> list[Any]:
		"""compute
		Args:
			frame (list[Any]): the input stereo audio signal. Defaults to . 
		Returns:
			psd (list[Any]): Power Spectral Density [dB] or [dB/Hz]
		""" 
		... 
...
class Windowing(_essentia.Algorithm): 
	...
	def __call__(self, frame:list[Any]) -> list[Any]:
		...
		Args:
			frame (list[Any]): the input audio frame. Defaults to . 
		Returns:
			frame (list[Any]): the windowed audio frame
		""" 
		... 
...
class ZeroCrossingRate(_essentia.Algorithm): 
	def __init__(self, threshold:float=0) -> None:
		...
		Args:
			threshold (float): the threshold which will be taken as the zero axis in both positive and negative sign. Defaults to 0. Range [0,inf] 
		""" 
		... 
	def __call__(self, signal:list[Any]) -> float:
		"""compute
