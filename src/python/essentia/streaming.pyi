...
class AfterMaxToBeforeMaxEnergyRatio(_essentia.Algorithm): 
	...
	def __call__(self, pitch:float) -> float:
		"""compute
		Args:
			pitch (float): the array of pitch values [Hz]. Defaults to None. 
		Returns:
...
class AllPass(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): the input signal. Defaults to None. 
		Returns:
...
class AudioLoader(_essentia.Algorithm): 
	def __init__(self, filename:str, audioStream:int=0, computeMD5:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, ) -> tuple[np.ndarray, float, int, str, int, str]:
		"""compute
		Returns:
...
class AudioWriter(_essentia.Algorithm): 
	...
	def __call__(self, audio:np.ndarray) -> None:
		"""compute
		Args:
			audio (np.ndarray): the input audio. Defaults to None. 
		""" 
		... 
...
class BandPass(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			signal (float): the filtered signal
		""" 
		... 
...
class BarkExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, hopSize:int=1024, sampleRate:float=44100.0) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[NDArray[np.float32], float, float, float, float, float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			barkbands (NDArray[np.float32]): spectral energy at each bark band. See BarkBands alogithm
...
class BeatTrackerDegara(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): input signal. Defaults to None. 
		Returns:
...
class BeatsLoudness(_essentia.Algorithm): 
	def __init__(self, beatDuration:float=0.05, beatWindowDuration:float=0.1, beats:NDArray[np.float32]=np.array([]), frequencyBands:NDArray[np.float32]=np.array([0, 200, 400, 800, 1600, 3200, 22000]), sampleRate:float=44100.0) -> None:
		...
		Args:
			...
			frequencyBands (NDArray[np.float32]): the list of frequency bands to compute energy ratios [Hz]. Defaults to np.array([0, 200, 400, 800, 1600, 3200, 22000]). Range None
			sampleRate (float): the audio sampling rate [Hz]. Defaults to 44100.0. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, NDArray[np.float32]]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			loudness (float): the beat's energy in the whole spectrum
			loudnessBandRatio (NDArray[np.float32]): the ratio of the beat's energy in each band
		""" 
		... 
...
class BinaryOperatorStream(_essentia.Algorithm): 
	...
	def __call__(self, array1:float, array2:float) -> float:
		"""compute
		Args:
			array1 (float): the first operand input array. Defaults to None. 
			array2 (float): the second operand input array. Defaults to None. 
		Returns:
...
class BpmHistogram(_essentia.Algorithm): 
	def __init__(self, bpm:float=0.0, constantTempo:bool=False, frameRate:float=86.1328, frameSize:float=4.0, maxBpm:float=560.0, maxPeaks:int=50, minBpm:float=30.0, overlap:int=16, tempoChange:float=5.0, weightByMagnitude:bool=True, windowType:str='hann', zeroPadding:int=0) -> None:
		...
		""" 
		... 
	def __call__(self, novelty:float) -> tuple[float, NDArray[np.float32], NDArray[np.float32], np.ndarray, NDArray[np.float32], NDArray[np.float32], NDArray[np.float32], NDArray[np.float32]]:
		"""compute
		Args:
			novelty (float): the novelty curve. Defaults to None. 
		Returns:
...
class ChordsDescriptors(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		... 
	def __call__(self, chords:str, key:str, scale:str) -> tuple[NDArray[np.float32], float, float, str, str]:
		"""compute
		Args:
			chords (str): the chord progression. Defaults to None. 
...
class ChordsDetection(_essentia.Algorithm): 
	def __init__(self, hopSize:int=2048, sampleRate:float=44100.0, windowSize:float=2.0) -> None:
		...
		""" 
		... 
	def __call__(self, pcp:NDArray[np.float32]) -> tuple[str, float]:
		"""compute
		Args:
			pcp (NDArray[np.float32]): the pitch class profile from which to detect the chord. Defaults to None. 
		Returns:
...
class ChromaCrossSimilarity(_essentia.Algorithm): 
	def __init__(self, binarizePercentile:float=0.095, frameStackSize:int=9, frameStackStride:int=1, oti:int=0, otiBinary:bool=False, referenceFeature:np.ndarray=np.array([])) -> None:
		...
		Args:
			...
			referenceFeature (np.ndarray): 2D matrix corresponds to the input chromagram of the reference song. (eg. a HPCP). Defaults to np.array([]). Range None 
		""" 
		... 
	def __call__(self, queryFeature:NDArray[np.float32]) -> NDArray[np.float32]:
		"""compute
		Args:
			queryFeature (NDArray[np.float32]): input chromagram of the query song. (eg: a HPCP). Defaults to None. 
		Returns:
			csm (NDArray[np.float32]): 2D binary cross-similarity matrix of the query and reference chromagram
		""" 
		... 
...
class CoverSongSimilarity(_essentia.Algorithm): 
	def __init__(self, disExtension:float=0.5, disOnset:float=0.5, distanceType:str='asymmetric', pipeDistance:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, inputArray:NDArray[np.float32]) -> tuple[np.ndarray, float]:
		"""compute
		Args:
			inputArray (NDArray[np.float32]): a 2D binary cross similarity matrix of two audio chroma vectors (refer CrossSimilarityMatrix algorithm').. Defaults to None. 
		Returns:
			scoreMatrix (np.ndarray): a 2D smith-waterman alignment score matrix from the input binary cross-similarity matrix as described in [2].
...
class FadeDetection(_essentia.Algorithm): 
	def __init__(self, cutoffHigh:float=0.85, cutoffLow:float=0.2, frameRate:float=4.0, minLength:float=3.0) -> None:
		...
		""" 
		... 
	def __call__(self, rms:float) -> tuple[np.ndarray, np.ndarray]:
		"""compute
		Args:
			rms (float): rms values array. Defaults to None. 
		Returns:
...
class FalseStereoDetector(_essentia.Algorithm): 
	def __init__(self, correlationThreshold:float=0.9995, frameSize:int=512, silenceThreshold:int=-70) -> None:
		...
		""" 
		... 
	def __call__(self, audio:np.ndarray) -> tuple[int, float]:
		"""compute
		Args:
			audio (np.ndarray): the input audio. Defaults to None. 
		Returns:
...
class FrameCutter(_essentia.Algorithm): 
	def __init__(self, frameSize:int=1024, hopSize:int=512, lastFrameToEndOfFile:bool=False, silentFrames:str='noise', startFromZero:bool=False, validFrameThresholdRatio:float=0.0) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> NDArray[np.float32]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			frame (NDArray[np.float32]): the frames of the audio signal
		""" 
		... 
...
class HighResolutionFeatures(_essentia.Algorithm): 
	...
	def __call__(self, hpcp:NDArray[np.float32]) -> tuple[float, float, float]:
		"""compute
		Args:
			hpcp (NDArray[np.float32]): the pitch class profile from which to detect the chord. Defaults to None. 
		Returns:
...
class HumDetector(_essentia.Algorithm): 
	def __init__(self, Q0:float=0.1, Q1:float=0.55, detectionThreshold:float=5.0, frameSize:float=0.4, hopSize:float=0.2, maximumFrequency:float=400.0, minimumDuration:float=0.5, minimumFrequency:float=27.5, numberHarmonics:int=1, sampleRate:float=44100.0, timeContinuity:float=10.0, timeWindow:float=9.0) -> None:
		...
		Args:
			...
			timeWindow (float): analysis time to use for the hum estimation [s]. Defaults to 9.0. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:float) -> tuple[np.ndarray, NDArray[np.float32], NDArray[np.float32], NDArray[np.float32], NDArray[np.float32]]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
...
class Key(_essentia.Algorithm): 
	def __init__(self, profileType:str='bgate', averageDetuningCorrection:bool=True, numHarmonics:int=4, pcpSize:int=36, pcpThreshold:float=0.2, slope:float=0.6, useMajMin:bool=False, usePolyphony:bool=True, useThreeChords:bool=True) -> None:
		...
		""" 
		... 
	def __call__(self, pcp:NDArray[np.float32]) -> tuple[str, str, float]:
		"""compute
		Args:
...
class KeyExtractor(_essentia.Algorithm): 
	...
	def __call__(self, audio:float) -> tuple[str, str, float]:
		"""compute
		Args:
			audio (float): the audio signal. Defaults to None. 
		Returns:
...
class Leq(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): the input signal (must be non-empty). Defaults to None. 
		Returns:
...
class LoudnessEBUR128(_essentia.Algorithm): 
	def __init__(self, hopSize:float=0.1, sampleRate:float=44100.0, startAtZero:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, signal:np.ndarray) -> tuple[float, float, float, float]:
		"""compute
		Args:
			signal (np.ndarray): the input stereo audio signal. Defaults to None. 
		Returns:
...
class LowLevelSpectralEqloudExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, hopSize:int=1024, sampleRate:float=44100.0) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, float, NDArray[np.float32], NDArray[np.float32], float, float, float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			...
			sccoeffs (NDArray[np.float32]): See SpectralContrast algorithm documentation
			scvalleys (NDArray[np.float32]): See SpectralContrast algorithm documentation
...
class LowLevelSpectralExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, hopSize:int=1024, sampleRate:float=44100.0) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[NDArray[np.float32], float, float, float, float, NDArray[np.float32], float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, NDArray[np.float32], float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			...
			mfcc (NDArray[np.float32]): See MFCC algorithm documentation
			...
			tristimulus (NDArray[np.float32]): See Tristimulus algorithm documentation
...
class MaxFilter(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> float:
		"""compute
		Args:
			signal (float): signal to be filtered. Defaults to None. 
		Returns:
...
class MaxToTotal(_essentia.Algorithm): 
	...
	def __call__(self, envelope:float) -> float:
		"""compute
		Args:
			envelope (float): the envelope of the signal. Defaults to None. 
		Returns:
...
class MetadataReader(_essentia.Algorithm): 
	def __init__(self, filename:str='', failOnError:bool=False) -> None:
...
class MonoMixer(_essentia.Algorithm): 
	def __init__(self, type:str='mix') -> None:
		...
		""" 
		... 
	def __call__(self, audio:np.ndarray, numberChannels:int) -> float:
		"""compute
		Args:
			audio (np.ndarray): the input stereo signal. Defaults to None. 
...
class MonoWriter(_essentia.Algorithm): 
	...
	def __call__(self, audio:float) -> None:
		"""compute
		Args:
			audio (float): the input audio. Defaults to None. 
		""" 
		... 
...
class MultiPitchMelodia(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10.0, filterIterations:int=3, frameSize:int=2048, guessUnvoiced:bool=False, harmonicWeight:float=0.8, hopSize:int=128, magnitudeCompression:float=1.0, magnitudeThreshold:int=40, maxFrequency:float=20000.0, minDuration:int=100, minFrequency:float=80.0, numberHarmonics:int=20, peakDistributionThreshold:float=0.9, peakFrameThreshold:float=0.9, pitchContinuity:float=27.5625, referenceFrequency:float=55.0, sampleRate:float=44100.0, timeContinuity:int=100) -> None:
		...
		Args:
			...
			maxFrequency (float): the maximum allowed frequency for salience function peaks (ignore peaks above) [Hz]. Defaults to 20000.0. Range [0,inf)
		...
		""" 
		... 
	def __call__(self, signal:float) -> np.ndarray:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			pitch (np.ndarray): the estimated pitch values per frames [Hz]
		""" 
		... 
...
class Multiplexer(_essentia.Algorithm): 
	...
	def __call__(self, ) -> NDArray[np.float32]:
		"""compute
		Returns:
			data (NDArray[np.float32]): the frame containing the input values and/or input frames
		""" 
		... 
...
class NSGConstantQStreaming(_essentia.Algorithm): 
	def __init__(self, binsPerOctave:int=48, gamma:int=0, inputSize:int=4096, maxFrequency:float=7040.0, minFrequency:float=27.5, minimumWindow:int=4, normalize:str='none', phaseMode:str='global', rasterize:str='full', sampleRate:float=44100.0, window:str='hannnsgcq', windowSizeFactor:int=1) -> None:
		...
		""" 
		... 
	def __call__(self, frame:NDArray[np.float32]) -> tuple[np.ndarray, np.ndarray, np.ndarray, int]:
		"""compute
		Args:
			frame (NDArray[np.float32]): the input audio signal. Defaults to None. 
		Returns:
			constantq (np.ndarray): the constant Q transform of the input frame
			constantqdc (np.ndarray): the DC band transform of the input frame. Only needed for the inverse transform
			constantqnf (np.ndarray): the Nyquist band transform of the input frame. Only needed for the inverse transform
...
class NoveltyCurve(_essentia.Algorithm): 
	def __init__(self, frameRate:float=344.531, normalize:bool=False, weightCurve:NDArray[np.float32]=np.array([]), weightCurveType:str='inverse_quadratic') -> None:
		...
		""" 
		... 
	def __call__(self, frequencyBands:NDArray[np.float32]) -> float:
		"""compute
		Args:
			frequencyBands (NDArray[np.float32]): the frequency bands. Defaults to None. 
		Returns:
...
class OnsetRate(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		... 
	def __call__(self, signal:float) -> tuple[NDArray[np.float32], float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			onsetTimes (NDArray[np.float32]): the detected onset times [s]
...
class PitchFilter(_essentia.Algorithm): 
	def __init__(self, confidenceThreshold:int=36, minChunkSize:int=30, useAbsolutePitchConfidence:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, pitchConfidence:NDArray[np.float32], pitch:NDArray[np.float32]) -> NDArray[np.float32]:
		"""compute
		Args:
...
class PitchMelodia(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10.0, filterIterations:int=3, frameSize:int=2048, guessUnvoiced:bool=False, harmonicWeight:float=0.8, hopSize:int=128, magnitudeCompression:float=1.0, magnitudeThreshold:int=40, maxFrequency:float=20000.0, minDuration:int=100, minFrequency:float=80.0, numberHarmonics:int=20, peakDistributionThreshold:float=0.9, peakFrameThreshold:float=0.9, pitchContinuity:float=27.5625, referenceFrequency:float=55.0, sampleRate:float=44100.0, timeContinuity:int=100) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[NDArray[np.float32], NDArray[np.float32]]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			pitch (NDArray[np.float32]): the estimated pitch values per frames [Hz]
			pitchConfidence (NDArray[np.float32]): the estimated pitch confidence
		""" 
		... 
...
class PitchYinProbabilistic(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> tuple[NDArray[np.float32], NDArray[np.float32]]:
		"""compute
		Args:
			signal (float): the input mono audio signal. Defaults to None. 
		Returns:
...
class PoolToTensor(_essentia.Algorithm): 
	def __init__(self, namespace:str='input_0') -> None:
		...
		""" 
		... 
	def __call__(self, pool:Pool) -> np.ndarray:
		"""compute
		Args:
			pool (Pool): the pool containing the required namespace. Defaults to None. 
		Returns:
			tensor (np.ndarray): tensors retrieved from the pool
		""" 
		... 
...
class RealAccumulator(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		... 
	def __call__(self, data:float) -> NDArray[np.float32]:
		"""compute
		Args:
			data (float): the input signal. Defaults to None. 
		Returns:
			array (NDArray[np.float32]): the accumulated signal in one single frame
		""" 
		... 


class ReplayGain(_essentia.Algorithm): 
	def __init__(self, applyEqloud:bool=True, sampleRate:float=44100.0) -> None:
...
class RhythmDescriptors(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		... 
	def __call__(self, signal:float) -> tuple[NDArray[np.float32], float, float, NDArray[np.float32], NDArray[np.float32], float, float, float, float, float, float, NDArray[np.float32]]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
...
class RhythmExtractor(_essentia.Algorithm): 
	def __init__(self, frameHop:int=1024, frameSize:int=1024, hopSize:int=256, lastBeatInterval:float=0.1, maxTempo:int=208, minTempo:int=40, numberFrames:int=1024, sampleRate:float=44100.0, tempoHints:NDArray[np.float32]=np.array([]), tolerance:float=0.24, useBands:bool=True, useOnset:bool=True) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, NDArray[np.float32], NDArray[np.float32], NDArray[np.float32]]:
		"""compute
		Args:
			signal (float): input signal. Defaults to None. 
		Returns:
...
class RhythmExtractor2013(_essentia.Algorithm): 
	def __init__(self, maxTempo:int=208, method:str='multifeature', minTempo:int=40) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[NDArray[np.float32], float, float, NDArray[np.float32], NDArray[np.float32]]:
		"""compute
		Args:
			signal (float): input signal. Defaults to None. 
		Returns:
...
class RhythmTransform(_essentia.Algorithm): 
	def __init__(self, frameSize:int=256, hopSize:int=32) -> None:
		...
		""" 
		... 
	def __call__(self, melBands:NDArray[np.float32]) -> np.ndarray:
		"""compute
		Args:
			melBands (NDArray[np.float32]): the energy in the melbands. Defaults to None. 
		Returns:
...
class Slicer(_essentia.Algorithm): 
	def __init__(self, endTimes:NDArray[np.float32]=np.array([]), sampleRate:float=44100.0, startTimes:NDArray[np.float32]=np.array([]), timeUnits:str='seconds') -> None:
		...
		""" 
		... 
	def __call__(self, audio:float) -> NDArray[np.float32]:
		"""compute
		Args:
			audio (float): the input signal. Defaults to None. 
		Returns:
			frame (NDArray[np.float32]): the frames of the sliced input signal
		""" 
		... 
...
class StereoDemuxer(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		... 
	def __call__(self, audio:np.ndarray) -> tuple[float, float]:
		"""compute
		Args:
			audio (np.ndarray): the input stereo signal. Defaults to None. 
		Returns:
...
class StereoMuxer(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		... 
	def __call__(self, left:float, right:float) -> np.ndarray:
		"""compute
		Args:
			left (float): the left channel of the audio signal. Defaults to None. 
			right (float): the right channel of the audio signal. Defaults to None. 
		Returns:
			audio (np.ndarray): the output stereo signal
		""" 
		... 


class StereoTrimmer(_essentia.Algorithm): 
	def __init__(self, endTime:float=1000000.0, sampleRate:float=44100.0, startTime:float=0.0) -> None:
	...
	def __call__(self, signal:np.ndarray) -> np.ndarray:
		"""compute
		Args:
			signal (np.ndarray): the input stereo signal. Defaults to None. 
		Returns:
			signal (np.ndarray): the trimmed stereo signal
		""" 
		... 
...
class SuperFluxExtractor(_essentia.Algorithm): 
	...
	def __call__(self, signal:float) -> NDArray[np.float32]:
		...
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			onsets (NDArray[np.float32]): lists of onsets
		""" 
		... 
...
class SuperFluxNovelty(_essentia.Algorithm): 
	def __init__(self, binWidth:int=3, frameWidth:int=2) -> None:
		...
		""" 
		... 
	def __call__(self, bands:NDArray[np.float32]) -> float:
		"""compute
		Args:
			bands (NDArray[np.float32]): the input bands spectrogram. Defaults to None. 
		Returns:
...
class SuperFluxPeaks(_essentia.Algorithm): 
	def __init__(self, combine:float=30.0, frameRate:float=172.0, pre_avg:float=100.0, pre_max:float=30.0, ratioThreshold:float=16.0, threshold:float=0.05) -> None:
		...
		Args:
			combine (float): ms for onset combination. Defaults to 30.0. Range (0,inf)
		...
		""" 
		... 
	def __call__(self, novelty:float) -> NDArray[np.float32]:
		"""compute
		Args:
			novelty (float): the input novelty. Defaults to None. 
		Returns:
			peaks (NDArray[np.float32]): peaks instants [s]
		""" 
		... 
...
class TCToTotal(_essentia.Algorithm): 
	...
	def __call__(self, envelope:float) -> float:
		"""compute
		Args:
			envelope (float): the envelope of the signal (its length must be greater than 1. Defaults to None. 
		Returns:
...
class TempoTapDegara(_essentia.Algorithm): 
	...
	def __call__(self, onsetDetections:float) -> float:
		"""compute
		Args:
			onsetDetections (float): per-frame onset detection values. Defaults to None. 
		Returns:
...
class TensorToPool(_essentia.Algorithm): 
	def __init__(self, mode:str='overwrite', namespace:str='input_0') -> None:
		...
		""" 
		... 
	def __call__(self, tensor:np.ndarray) -> Pool:
		"""compute
		Args:
			tensor (np.ndarray): the tensor to be added to the pool. Defaults to None. 
		Returns:
...
class TensorToVectorReal(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		... 
	def __call__(self, tensor:np.ndarray) -> NDArray[np.float32]:
		"""compute
		Args:
			tensor (np.ndarray): the input tensor. Defaults to None. 
		Returns:
			frame (NDArray[np.float32]): the frames to be retrieved from the tensor
		""" 
		... 
...
class TonalExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=4096, hopSize:int=2048, tuningFrequency:float=440.0) -> None:
		...
		""" 
		... 
	def __call__(self, signal:float) -> tuple[float, NDArray[np.float32], str, float, str, str, float, NDArray[np.float32], NDArray[np.float32], str, str, float]:
		"""compute
		Args:
			signal (float): the input audio signal. Defaults to None. 
		Returns:
			...
			hpcp (NDArray[np.float32]): See HPCP algorithm documentation
			hpcp_highres (NDArray[np.float32]): See HPCP algorithm documentation
...
class UnaryOperatorStream(_essentia.Algorithm): 
	...
	def __call__(self, array:float) -> float:
		"""compute
		Args:
			array (float): the input array. Defaults to None. 
		Returns:
...
class VectorRealAccumulator(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		"""Takes a stream of Real values and outputs them as a single vector when the end of the stream is reached.		""" 
		... 
	def __call__(self, data:NDArray[np.float32]) -> np.ndarray:
		"""compute
		Args:
			data (NDArray[np.float32]): the input signal. Defaults to None. 
		Returns:
			array (np.ndarray): the accumulated signal in one single frame
		""" 
		... 


class VectorRealToTensor(_essentia.Algorithm): 
	def __init__(self, batchHopSize:int=0, lastBatchMode:str='push', lastPatchMode:str='repeat', patchHopSize:int=0, shape:np.ndarray=np.array([])) -> None:
		...
		Args:
			...
			shape (np.ndarray): shape of the output tensor (batchSize, channels, patchSize, featureSize). If batchSize is set to -1 or 0 a single tensor is generated when the end of the stream is reached. Defaults to np.array([]). Range None 
		""" 
		... 
	def __call__(self, frame:NDArray[np.float32]) -> np.ndarray:
		"""compute
		Args:
			frame (NDArray[np.float32]): the input frames. Defaults to None. 
		Returns:
			tensor (np.ndarray): the accumulated frame in one single tensor
		""" 
		... 
