...
class AfterMaxToBeforeMaxEnergyRatio(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, pitch:list[Any]) -> float:
		"""compute
		Args:
			pitch (list[Any]): the array of pitch values [Hz]. Defaults to . 
		Returns:
...
class AllPass(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Returns:
			signal (list[Any]): the filtered signal
		""" 
		... 
...
class Audio2Midi(_essentia.Algorithm): 
	def __init__(self, applyTimeCompensation:bool=True, hopSize:int=32, loudnessThreshold:float=-51, maxFrequency:float=2300, midiBufferDuration:float=0.05, minFrequency:float=60, minNoteChangePeriod:float=0.03, minOccurrenceRate:float=0.5, minOffsetCheckPeriod:float=0.2, minOnsetCheckPeriod:float=0.075, pitchConfidenceThreshold:float=0.25, sampleRate:int=44100, transpositionAmount:int=0, tuningFrequency:int=440) -> None:
		...
		Args:
			...
			loudnessThreshold (float): loudness level above/below which note ON/OFF start to be considered, in decibels. Defaults to -51. Range [-inf,0]
			maxFrequency (float): maximum frequency to detect in Hz. Defaults to 2300. Range [10,20000]
			...
			minFrequency (float): minimum frequency to detect in Hz. Defaults to 60. Range [10,20000]
		...
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[float, float, str, list[Any], list[Any]]:
		"""compute
		Args:
			frame (list[Any]): the input frame to analyse. Defaults to . 
		Returns:
			pitch (float): pitch given in Hz
			loudness (float): detected loudness in decibels
			messageType (str): the output of MIDI message type, as string, {noteoff, noteon, noteoff-noteon}
			midiNoteNumber (list[Any]): the output of detected MIDI note number, as integer, in range [0,127]
			timeCompensation (list[Any]): time to be compensated in the messages
		""" 
		... 
...
class Audio2Pitch(_essentia.Algorithm): 
	def __init__(self, frameSize:int=1024, loudnessThreshold:float=-51, maxFrequency:float=2300, minFrequency:float=60, pitchAlgorithm:str='pitchyinfft', pitchConfidenceThreshold:float=0.25, sampleRate:int=44100, tolerance:float=1, weighting:str='custom') -> None:
		...
		Args:
			...
			sampleRate (int): sample rate of incoming audio frames. Defaults to 44100. Range [8000,inf)
			tolerance (float): sets tolerance for peak detection on pitch algorithm. Defaults to 1. Range [0,1]
			weighting (str): string to assign a weighting function. Defaults to 'custom'. Range {custom,A,B,C,D,Z} 
		""" 
		... 
	def __call__(self, frame:list[Any]) -> tuple[float, float, float, int]:
		"""compute
		Args:
			frame (list[Any]): the input frame to analyse. Defaults to . 
		Returns:
...
class AudioOnsetsMarker(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			signal (list[Any]): the input signal mixed with bursts at onset locations
		""" 
		... 
...
class AudioWriter(_essentia.Algorithm): 
	...
	def __call__(self, audio:ndarray) -> None:
		"""compute
		Args:
			audio (ndarray): the audio signal. Defaults to . 
		Returns:
None
		""" 
		... 
...
class BeatTrackerMultiFeature(_essentia.Algorithm): 
	def __init__(self, maxTempo:int=208, minTempo:int=40) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[list[Any], float]:
		"""compute
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
...
class BeatsLoudness(_essentia.Algorithm): 
	def __init__(self, beatDuration:float=0.05, beatWindowDuration:float=0.1, beats:list[Any]=[], frequencyBands:list[Any]=[20, 150, 400, 3200, 7000, 22000], sampleRate:float=44100) -> None:
		...
		Args:
			...
			frequencyBands (list[Any]): the list of bands to compute energy ratios [Hz. Defaults to [20, 150, 400, 3200, 7000, 22000]. Range None
			sampleRate (float): the audio sampling rate [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[list[Any], ndarray]:
		"""compute
		Args:
			signal (list[Any]): the input audio signal. Defaults to . 
		Returns:
			loudness (list[Any]): the beat's energy in the whole spectrum
			loudnessBandRatio (ndarray): the ratio of the beat's energy on each frequency band
		""" 
		... 
...
class BpmHistogram(_essentia.Algorithm): 
	def __init__(self, bpm:float=0, constantTempo:bool=False, frameRate:float=86.1328, frameSize:float=4, maxBpm:float=560, maxPeaks:int=50, minBpm:float=30, overlap:int=16, tempoChange:float=5, weightByMagnitude:bool=True, windowType:str='hann', zeroPadding:int=0) -> None:
		...
		""" 
		... 
	def __call__(self, novelty:list[Any]) -> tuple[float, list[Any], list[Any], ndarray, list[Any], list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			novelty (list[Any]): the novelty curve. Defaults to . 
		Returns:
...
class ChordsDetection(_essentia.Algorithm): 
	def __init__(self, hopSize:int=2048, sampleRate:float=44100, windowSize:float=2) -> None:
		...
		""" 
		... 
	def __call__(self, pcp:ndarray) -> tuple[str, list[Any]]:
		"""compute
		Args:
			pcp (ndarray): the pitch class profile from which to detect the chord. Defaults to . 
		Returns:
			chords (str): the resulting chords, from A to G
			strength (list[Any]): the strength of the chord
		""" 
		... 
...
class ChordsDetectionBeats(_essentia.Algorithm): 
	def __init__(self, chromaPick:str='interbeat_median', hopSize:int=2048, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, pcp:ndarray, ticks:list[Any]) -> tuple[str, list[Any]]:
		"""compute
		Args:
			pcp (ndarray): the pitch class profile from which to detect the chord. Defaults to . 
			ticks (list[Any]): the list of beat positions (in seconds). One chord will be outputted for each segment between two adjacent ticks. If number of ticks is smaller than 2, exception will be thrown. Those ticks that exceeded the pcp time length will be ignored.. Defaults to . 
		Returns:
			chords (str): the resulting chords, from A to G
			strength (list[Any]): the strength of the chords
		""" 
		... 
...
class ChromaCrossSimilarity(_essentia.Algorithm): 
	def __init__(self, binarizePercentile:float=0.095, frameStackSize:int=9, frameStackStride:int=1, noti:int=12, oti:bool=True, otiBinary:bool=False, streaming:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, queryFeature:ndarray, referenceFeature:ndarray) -> ndarray:
		"""compute
		Args:
			queryFeature (ndarray): frame-wise chromagram of the query song (e.g., a HPCP). Defaults to . 
			referenceFeature (ndarray): frame-wise chromagram of the reference song (e.g., a HPCP). Defaults to . 
		Returns:
			csm (ndarray): 2D binary cross-similarity matrix of the query and reference features
		""" 
		... 
...
class Clipper(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			signal (list[Any]): the output signal with the added noise
		""" 
		... 
...
class CoverSongSimilarity(_essentia.Algorithm): 
	def __init__(self, alignmentType:str='serra09', disExtension:float=0.5, disOnset:float=0.5, distanceType:str='asymmetric') -> None:
		...
		""" 
		... 
	def __call__(self, inputArray:ndarray) -> tuple[ndarray, float]:
		"""compute
		Args:
			inputArray (ndarray):  a 2D binary cross-similarity matrix between two audio chroma vectors (query vs reference song) (refer 'ChromaCrossSimilarity' algorithm').. Defaults to . 
		Returns:
			scoreMatrix (ndarray): a 2D smith-waterman alignment score matrix from the input binary cross-similarity matrix
...
class CrossSimilarityMatrix(_essentia.Algorithm): 
	...
	def __call__(self, queryFeature:ndarray, referenceFeature:ndarray) -> ndarray:
		"""compute
		Args:
			queryFeature (ndarray): input frame features of the query song (e.g., a chromagram). Defaults to . 
			referenceFeature (ndarray): input frame features of the reference song (e.g., a chromagram). Defaults to . 
		Returns:
			csm (ndarray): 2D cross-similarity matrix of two input frame sequences (query vs reference)
		""" 
		... 
...
class DCRemoval(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input audio signal. Defaults to . 
		Returns:
			signal (list[Any]): the filtered signal, with the DC component removed
		""" 
		... 
...
class Danceability(_essentia.Algorithm): 
	def __init__(self, maxTau:float=8800, minTau:float=310, sampleRate:float=44100, tauMultiplier:float=1.1) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[float, list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
...
class Derivative(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			signal (list[Any]): the derivative of the input signal
		""" 
		... 
...
class EasyLoader(_essentia.Algorithm): 
	...
	def __call__(self, ) -> list[Any]:
		...
		Returns:
			audio (list[Any]): the audio signal
		""" 
		... 
...
class Envelope(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			signal (list[Any]): the resulting envelope of the signal
		""" 
		... 
...
class Extractor(_essentia.Algorithm): 
	def __init__(self, dynamics:bool=True, dynamicsFrameSize:int=88200, dynamicsHopSize:int=44100, highLevel:bool=True, lowLevel:bool=True, lowLevelFrameSize:int=2048, lowLevelHopSize:int=1024, midLevel:bool=True, namespace:str="", relativeIoi:bool=False, rhythm:bool=True, sampleRate:float=44100, tonalFrameSize:int=4096, tonalHopSize:int=2048, tuning:bool=True) -> None:
		...
		""" 
		... 
	def __call__(self, audio:list[Any]) -> Pool:
		"""compute
		Args:
			audio (list[Any]): the input audio signal. Defaults to . 
		Returns:
...
class FadeDetection(_essentia.Algorithm): 
	def __init__(self, cutoffHigh:float=0.85, cutoffLow:float=0.2, frameRate:float=4, minLength:float=3) -> None:
		...
		""" 
		... 
	def __call__(self, rms:list[Any]) -> tuple[ndarray, ndarray]:
		"""compute
		Args:
			rms (list[Any]): rms values array. Defaults to . 
		Returns:
...
class FalseStereoDetector(_essentia.Algorithm): 
	def __init__(self, correlationThreshold:float=0.9995, silenceThreshold:int=-70) -> None:
		...
		""" 
		... 
	def __call__(self, frame:ndarray) -> tuple[int, float]:
		"""compute
		Args:
			frame (ndarray): the input frame (must be non-empty). Defaults to . 
		Returns:
...
class FrameCutter(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		"""compute
		Args:
			signal (list[Any]): the buffer from which to read data. Defaults to . 
		Returns:
			frame (list[Any]): the frame to write to
		""" 
		... 
...
class FrameToReal(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Returns:
			signal (list[Any]): the output audio samples
		""" 
		... 


class FreesoundExtractor(_essentia.Algorithm): 
	def __init__(self, analysisSampleRate:float=44100, endTime:float=1e+06, gfccStats:str='["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]', lowlevelFrameSize:int=2048, lowlevelHopSize:int=1024, lowlevelSilentFrames:str='noise', lowlevelStats:str='["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]', lowlevelWindowType:str='blackmanharris62', lowlevelZeroPadding:int=0, mfccStats:str='["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]', profile:str='None', rhythmMaxTempo:int=210, rhythmMethod:str='multifeature', rhythmMinTempo:int=40, rhythmStats:str='["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]', startTime:float=0, tonalFrameSize:int=4096, tonalHopSize:int=2048, tonalSilentFrames:str='noise', tonalStats:str='["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]', tonalWindowType:str='blackmanharris62', tonalZeroPadding:int=0) -> None:
		"""Is a wrapper for Freesound Extractor.
		
		See documentation for 'essentia_streaming_extractor_freesound'. 

		Args:
			analysisSampleRate (float): the analysis sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			endTime (float): the end time of the slice you want to extract [s]. Defaults to 1e+06. Range [0,inf)
			gfccStats (str): the statistics to compute for GFCC features. Defaults to '["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]'. Range None
			lowlevelFrameSize (int): the frame size for computing low-level features. Defaults to 2048. Range (0,inf)
			lowlevelHopSize (int): the hop size for computing low-level features. Defaults to 1024. Range (0,inf)
			...
			lowlevelStats (str): the statistics to compute for low-level features. Defaults to '["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]'. Range None
			...
			mfccStats (str): the statistics to compute for MFCC features. Defaults to '["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]'. Range None
			...
			rhythmStats (str): the statistics to compute for rhythm features. Defaults to '["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]'. Range None
			...
			tonalStats (str): the statistics to compute for tonal features. Defaults to '["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]'. Range None
	...
	def __call__(self, filename:str) -> tuple[Pool, Pool]:
		"""compute
		Args:
			filename (str): the input audiofile. Defaults to . 
		Returns:
...
class HighResolutionFeatures(_essentia.Algorithm): 
	...
	def __call__(self, hpcp:list[Any]) -> tuple[float, float, float]:
		"""compute
		Args:
			hpcp (list[Any]): the HPCPs, preferably of size >= 120. Defaults to . 
		Returns:
...
class HumDetector(_essentia.Algorithm): 
	def __init__(self, Q0:float=0.1, Q1:float=0.55, detectionThreshold:float=5, frameSize:float=0.4, hopSize:float=0.2, maximumFrequency:float=400, minimumDuration:float=2, minimumFrequency:float=22.5, numberHarmonics:int=1, sampleRate:float=44100, timeContinuity:float=10, timeWindow:float=10) -> None:
		...
		Args:
			...
			minimumDuration (float): minimun duration of the humming tones [s]. Defaults to 2. Range (0,inf)
			...
			timeWindow (float): analysis time to use for the hum estimation [s]. Defaults to 10. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[ndarray, list[Any], list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the input audio signal. Defaults to . 
		Returns:
...
class Intensity(_essentia.Algorithm): 
	def __init__(self, sampleRate:float=44100) -> None:
		...
		Args:
			sampleRate (float): the input audio sampling rate [Hz]. Defaults to 44100. Range (0,inf) 
		""" 
		... 
	def __call__(self, signal:list[Any]) -> int:
		"""compute
		Args:
			signal (list[Any]): the input audio signal. Defaults to . 
		Returns:
...
class Key(_essentia.Algorithm): 
	def __init__(self, numHarmonics:int=4, pcpSize:int=36, profileType:str='bgate', slope:float=0.6, useMajMin:bool=False, usePolyphony:bool=True, useThreeChords:bool=True) -> None:
		...
		""" 
		... 
	def __call__(self, pcp:list[Any]) -> tuple[str, str, float, float]:
		"""compute
		Args:
...
class KeyExtractor(_essentia.Algorithm): 
	def __init__(self, averageDetuningCorrection:bool=True, frameSize:int=4096, hopSize:int=4096, hpcpSize:int=12, maxFrequency:float=3500, maximumSpectralPeaks:int=60, minFrequency:float=25, pcpThreshold:float=0.2, profileType:str='bgate', sampleRate:float=44100, spectralPeaksThreshold:float=0.0001, tuningFrequency:float=440, weightType:str='cosine', windowType:str='hann') -> None:
		...
		""" 
		... 
	def __call__(self, audio:list[Any]) -> tuple[str, str, float]:
		"""compute
		Args:
			audio (list[Any]): the audio input signal. Defaults to . 
		Returns:
...
class Leq(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> float:
		"""compute
		Args:
			signal (list[Any]): the input signal (must be non-empty). Defaults to . 
		Returns:
...
class LevelExtractor(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
			loudness (list[Any]): the loudness values
		""" 
		... 
...
class LoudnessEBUR128(_essentia.Algorithm): 
	def __init__(self, hopSize:float=0.1, sampleRate:float=44100, startAtZero:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, signal:ndarray) -> tuple[list[Any], list[Any], float, float]:
		"""compute
		Args:
			signal (ndarray): the input stereo audio signal. Defaults to . 
		Returns:
			momentaryLoudness (list[Any]): momentary loudness (over 400ms) (LUFS)
			shortTermLoudness (list[Any]): short-term loudness (over 3 seconds) (LUFS)
...
class LowLevelSpectralEqloudExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, hopSize:int=1024, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[list[Any], ndarray, ndarray, list[Any], list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the input audio signal. Defaults to . 
		Returns:
			dissonance (list[Any]): See Dissonance algorithm documentation
			sccoeffs (ndarray): See SpectralContrast algorithm documentation
			scvalleys (ndarray): See SpectralContrast algorithm documentation
			spectral_centroid (list[Any]): See Centroid algorithm documentation
			spectral_kurtosis (list[Any]): See DistributionShape algorithm documentation
			spectral_skewness (list[Any]): See DistributionShape algorithm documentation
			spectral_spread (list[Any]): See DistributionShape algorithm documentation
		""" 
		... 
...
class LowLevelSpectralExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=2048, hopSize:int=1024, sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[ndarray, list[Any], list[Any], list[Any], list[Any], ndarray, list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], list[Any], ndarray, list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
			barkbands (ndarray): spectral energy at each bark band. See BarkBands alogithm
			barkbands_kurtosis (list[Any]): kurtosis from bark bands. See DistributionShape algorithm documentation
			barkbands_skewness (list[Any]): skewness from bark bands. See DistributionShape algorithm documentation
			barkbands_spread (list[Any]): spread from barkbands. See DistributionShape algorithm documentation
			hfc (list[Any]): See HFC algorithm documentation
			mfcc (ndarray): See MFCC algorithm documentation
			pitch (list[Any]): See PitchYinFFT algorithm documentation
			pitch_instantaneous_confidence (list[Any]): See PitchYinFFT algorithm documentation
			pitch_salience (list[Any]): See PitchSalience algorithm documentation
			silence_rate_20dB (list[Any]): See SilenceRate algorithm documentation
			silence_rate_30dB (list[Any]): See SilenceRate algorithm documentation
			silence_rate_60dB (list[Any]): See SilenceRate algorithm documentation
			spectral_complexity (list[Any]): See Spectral algorithm documentation
			spectral_crest (list[Any]): See Crest algorithm documentation
			spectral_decrease (list[Any]): See Decrease algorithm documentation
			spectral_energy (list[Any]): See Energy algorithm documentation
			spectral_energyband_low (list[Any]): Energy in band (20,150] Hz. See EnergyBand algorithm documentation
			spectral_energyband_middle_low (list[Any]): Energy in band (150,800] Hz.See EnergyBand algorithm documentation
			spectral_energyband_middle_high (list[Any]): Energy in band (800,4000] Hz. See EnergyBand algorithm documentation
			spectral_energyband_high (list[Any]): Energy in band (4000,20000] Hz. See EnergyBand algorithm documentation
			spectral_flatness_db (list[Any]): See flatnessDB algorithm documentation
			spectral_flux (list[Any]): See Flux algorithm documentation
			spectral_rms (list[Any]): See RMS algorithm documentation
			spectral_rolloff (list[Any]): See RollOff algorithm documentation
			spectral_strongpeak (list[Any]): See StrongPeak algorithm documentation
			zerocrossingrate (list[Any]): See ZeroCrossingRate algorithm documentation
			inharmonicity (list[Any]): See Inharmonicity algorithm documentation
			tristimulus (ndarray): See Tristimulus algorithm documentation
			oddtoevenharmonicenergyratio (list[Any]): See OddToEvenHarmonicEnergyRatio algorithm documentation
		""" 
		... 
...
class MaxFilter(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		"""compute
		Args:
			signal (list[Any]): signal to be filtered. Defaults to . 
		Returns:
			signal (list[Any]): filtered output
		""" 
		... 
...
class MetadataReader(_essentia.Algorithm): 
	def __init__(self, failOnError:bool=False, filename:str='None', filterMetadata:bool=False, filterMetadataTags:str='[]', tagPoolName:str='metadata.tags') -> None:
		...
		Args:
			...
			filterMetadataTags (str): the list of tags to whitelist (original taglib names). Defaults to '[]'. Range None
...
class MonoMixer(_essentia.Algorithm): 
	def __init__(self, type:str='mix') -> None:
		...
		""" 
		... 
	def __call__(self, audio:ndarray, numberChannels:int) -> list[Any]:
		"""compute
		Args:
		...
		Returns:
			audio (list[Any]): the downmixed signal
		""" 
		... 
...
class MonoWriter(_essentia.Algorithm): 
	def __init__(self, bitrate:int=192, filename:str='None', format:str='wav', sampleRate:float=44100) -> None:
		...
		""" 
		... 
	def __call__(self, audio:list[Any]) -> None:
		"""compute
		Args:
			audio (list[Any]): the audio signal. Defaults to . 
		Returns:
None
		""" 
		... 
...
class MultiPitchKlapuri(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, frameSize:int=2048, harmonicWeight:float=0.8, hopSize:int=128, magnitudeCompression:float=1, magnitudeThreshold:int=40, maxFrequency:float=1760, minFrequency:float=80, numberHarmonics:int=10, referenceFrequency:float=55, sampleRate:float=44100) -> None:
		...
		Args:
			...
			magnitudeCompression (float): magnitude compression parameter for the salience function (=0 for maximum compression, =1 for no compression). Defaults to 1. Range (0,1]
			...
			maxFrequency (float): the maximum allowed frequency for salience function peaks (ignore peaks above) [Hz]. Defaults to 1760. Range [0,inf)
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> ndarray:
		"""compute
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			pitch (ndarray): the estimated pitch values [Hz]
		""" 
		... 
...
class MultiPitchMelodia(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, filterIterations:int=3, frameSize:int=2048, guessUnvoiced:bool=False, harmonicWeight:float=0.8, hopSize:int=128, magnitudeCompression:float=1, magnitudeThreshold:int=40, maxFrequency:float=20000, minDuration:int=100, minFrequency:float=40, numberHarmonics:int=20, peakDistributionThreshold:float=0.9, peakFrameThreshold:float=0.9, pitchContinuity:float=27.5625, referenceFrequency:float=55, sampleRate:float=44100, timeContinuity:int=100) -> None:
		...
		Args:
			...
			minFrequency (float): the minimum allowed frequency for salience function peaks (ignore contours with peaks below) [Hz]. Defaults to 40. Range [0,inf)
...
class Multiplexer(_essentia.Algorithm): 
	...
	def __call__(self, ) -> ndarray:
		...
		Returns:
			data (ndarray): the frame containing the input values and/or input frames
		""" 
		... 


class MusicExtractor(_essentia.Algorithm): 
	def __init__(self, analysisSampleRate:float=44100, endTime:float=1e+06, gfccStats:str='["mean", "cov", "icov"]', loudnessFrameSize:int=88200, loudnessHopSize:int=44100, lowlevelFrameSize:int=2048, lowlevelHopSize:int=1024, lowlevelSilentFrames:str='noise', lowlevelStats:str='["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]', lowlevelWindowType:str='blackmanharris62', lowlevelZeroPadding:int=0, mfccStats:str='["mean", "cov", "icov"]', profile:str='None', requireMbid:bool=False, rhythmMaxTempo:int=208, rhythmMethod:str='degara', rhythmMinTempo:int=40, rhythmStats:str='["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]', startTime:float=0, tonalFrameSize:int=4096, tonalHopSize:int=2048, tonalSilentFrames:str='noise', tonalStats:str='["mean", "var", "stdev", "median", "min", "max", "dmean", "dmean2", "dvar", "dvar2"]', tonalWindowType:str='blackmanharris62', tonalZeroPadding:int=0) -> None:
		...
		Args:
			analysisSampleRate (float): the analysis sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			endTime (float): the end time of the slice you want to extract [s]. Defaults to 1e+06. Range [0,inf)
			gfccStats (str): the statistics to compute for GFCC features. Defaults to '["mean", "cov", "icov"]'. Range None
			...
			lowlevelWindowType (str): the window type for computing low-level features. Defaults to 'blackmanharris62'. Range {hamming,hann,triangular,square,blackmanharris62,blackmanharris70,blackmanharris74,blackmanharris92}
			lowlevelZeroPadding (int): zero padding factor for computing low-level features. Defaults to 0. Range [0,inf)
			mfccStats (str): the statistics to compute for MFCC features. Defaults to '["mean", "cov", "icov"]'. Range None
			profile (str): profile filename. If specified, default parameter values are overwritten by values in the profile yaml file. If not specified (empty string), use values configured by user like in other normal algorithms. Defaults to 'None'. Range None
...
class NoveltyCurve(_essentia.Algorithm): 
	def __init__(self, frameRate:float=344.531, normalize:bool=False, weightCurve:list[Any]=[], weightCurveType:str='hybrid') -> None:
		...
		""" 
		... 
	def __call__(self, frequencyBands:ndarray) -> list[Any]:
		"""compute
		Args:
			frequencyBands (ndarray): the frequency bands. Defaults to . 
		Returns:
			novelty (list[Any]): the novelty curve as a single vector
		""" 
		... 
...
class NoveltyCurveFixedBpmEstimator(_essentia.Algorithm): 
	def __init__(self, hopSize:int=512, maxBpm:float=560, minBpm:float=30, sampleRate:float=44100, tolerance:float=3) -> None:
		...
		Args:
			...
			maxBpm (float): the maximum bpm to look for. Defaults to 560. Range (0,inf)
			minBpm (float): the minimum bpm to look for. Defaults to 30. Range (0,inf)
			sampleRate (float): the sampling rate original audio signal [Hz]. Defaults to 44100. Range [1,inf)
			tolerance (float): tolerance (in percentage) for considering bpms to be equal. Defaults to 3. Range (0,100] 
		""" 
		... 
	def __call__(self, novelty:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			novelty (list[Any]): the novelty curve of the audio signal. Defaults to . 
		Returns:
			bpms (list[Any]): the bpm candidates sorted by magnitude
			amplitudes (list[Any]): the magnitude of each bpm candidate
		""" 
		... 
...
class OnsetDetectionGlobal(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			onsetDetections (list[Any]): the frame-wise values of the detection function
		""" 
		... 
...
class OnsetRate(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> tuple[list[Any], float]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			onsets (list[Any]): the positions of detected onsets [s]
...
class OverlapAdd(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		"""compute
		Args:
			signal (list[Any]): the windowed input audio frame. Defaults to . 
		Returns:
			signal (list[Any]): the output overlap-add audio signal frame
		""" 
		... 
...
class PCA(_essentia.Algorithm): 
	...
	def __call__(self, poolIn:Pool) -> Pool:
		"""compute
		Args:
			poolIn (Pool): the pool where to get the spectral contrast feature vectors. Defaults to . 
		Returns:
...
class PercivalBpmEstimator(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> float:
		"""compute
		Args:
			signal (list[Any]): input signal. Defaults to . 
		Returns:
...
class Pitch2Midi(_essentia.Algorithm): 
	def __init__(self, applyTimeCompensation:bool=True, hopSize:int=128, midiBufferDuration:float=0.015, minFrequency:float=60, minNoteChangePeriod:float=0.03, minOccurrenceRate:float=0.5, minOffsetCheckPeriod:float=0.2, minOnsetCheckPeriod:float=0.075, sampleRate:int=44100, transpositionAmount:int=0, tuningFrequency:int=440) -> None:
		...
		Args:
			...
			minFrequency (float): minimum detectable frequency. Defaults to 60. Range [20,20000]
		...
		""" 
		... 
	def __call__(self, pitch:float, voiced:int) -> tuple[str, list[Any], list[Any]]:
		"""compute
		Args:
			pitch (float): pitch given in Hz for conversion. Defaults to . 
			voiced (int): whether the frame is voiced or not, (0, 1). Defaults to . 
		Returns:
			messageType (str): the output of MIDI message type, as string, {noteoff, noteon, noteoff-noteon}
			midiNoteNumber (list[Any]): the output of detected MIDI note number, as integer, in range [0,127]
			timeCompensation (list[Any]): time to be compensated in the messages
...
class PitchContourSegmentation(_essentia.Algorithm): 
	def __init__(self, hopSize:int=128, minDuration:float=0.1, pitchDistanceThreshold:int=60, rmsThreshold:int=-2, sampleRate:int=44100, tuningFrequency:int=440) -> None:
		...
		""" 
		... 
	def __call__(self, pitch:list[Any], signal:list[Any]) -> tuple[list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			pitch (list[Any]): estimated pitch contour [Hz]. Defaults to . 
			signal (list[Any]): input audio signal. Defaults to . 
		Returns:
			onset (list[Any]): note onset times [s]
			duration (list[Any]): note durations [s]
			MIDIpitch (list[Any]): quantized MIDI pitch value
		""" 
		... 
...
class PitchFilter(_essentia.Algorithm): 
	def __init__(self, confidenceThreshold:int=36, minChunkSize:int=30, useAbsolutePitchConfidence:bool=False) -> None:
		...
		""" 
		... 
	def __call__(self, pitch:list[Any], pitchConfidence:list[Any]) -> list[Any]:
		"""compute
		Args:
...
class PitchMelodia(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> tuple[list[Any], list[Any]]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			pitch (list[Any]): the estimated pitch values [Hz]
			pitchConfidence (list[Any]): confidence with which the pitch was detected
		""" 
		... 
...
class PitchYinProbabilistic(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the input mono audio signal. Defaults to . 
		Returns:
...
class PredominantPitchMelodia(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, filterIterations:int=3, frameSize:int=2048, guessUnvoiced:bool=False, harmonicWeight:float=0.8, hopSize:int=128, magnitudeCompression:float=1, magnitudeThreshold:int=40, maxFrequency:float=20000, minDuration:int=100, minFrequency:float=80, numberHarmonics:int=20, peakDistributionThreshold:float=0.9, peakFrameThreshold:float=0.9, pitchContinuity:float=27.5625, referenceFrequency:float=55, sampleRate:float=44100, timeContinuity:int=100, voiceVibrato:bool=False, voicingTolerance:float=0.2) -> None:
		...
		Args:
			...
			peakFrameThreshold (float): per-frame salience threshold factor (fraction of the highest peak salience in a frame). Defaults to 0.9. Range [0,1]
			pitchContinuity (float): pitch continuity cue (maximum allowed pitch change during 1 ms time period) [cents]. Defaults to 27.5625. Range [0,inf)
			referenceFrequency (float): the reference frequency for Hertz to cent conversion [Hz], corresponding to the 0th cent bin. Defaults to 55. Range (0,inf)
			sampleRate (float): the sampling rate of the audio signal [Hz]. Defaults to 44100. Range (0,inf)
			timeContinuity (int): time continuity cue (the maximum allowed gap duration for a pitch contour) [ms]. Defaults to 100. Range (0,inf)
			voiceVibrato (bool): detect voice vibrato. Defaults to False. Range {true,false}
			voicingTolerance (float): allowed deviation below the average contour mean salience of all contours (fraction of the standard deviation). Defaults to 0.2. Range [-1.0,1.4] 
...
class ReplayGain(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> float:
		"""compute
		Args:
			signal (list[Any]): the input audio signal (must be longer than 0.05ms). Defaults to . 
		Returns:
...
class Resample(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			signal (list[Any]): the resampled signal
		""" 
		... 
...
class RhythmDescriptors(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[list[Any], float, float, list[Any], list[Any], float, float, float, float, float, float, list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
...
class RhythmExtractor(_essentia.Algorithm): 
	def __init__(self, frameHop:int=1024, frameSize:int=1024, hopSize:int=256, lastBeatInterval:float=0.1, maxTempo:int=208, minTempo:int=40, numberFrames:int=1024, sampleRate:float=44100, tempoHints:list[Any]=[], tolerance:float=0.24, useBands:bool=True, useOnset:bool=True) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[float, list[Any], list[Any], list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
			bpm (float): the tempo estimation [bpm]
			ticks (list[Any]):  the estimated tick locations [s]
			estimates (list[Any]): the bpm estimation per frame [bpm]
...
class RhythmExtractor2013(_essentia.Algorithm): 
	def __init__(self, maxTempo:int=208, method:str='multifeature', minTempo:int=40) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[float, list[Any], float, list[Any], list[Any]]:
		"""compute
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
...
class RhythmTransform(_essentia.Algorithm): 
	def __init__(self, frameSize:int=256, hopSize:int=32) -> None:
		...
		""" 
		... 
	def __call__(self, melBands:ndarray) -> ndarray:
		"""compute
		Args:
			melBands (ndarray): the energies in the mel bands. Defaults to . 
		Returns:
...
class Scale(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input audio signal. Defaults to . 
		Returns:
			signal (list[Any]): the output audio signal
		""" 
		... 
...
class Slicer(_essentia.Algorithm): 
	def __init__(self, endTimes:list[Any]=[], sampleRate:float=44100, startTimes:list[Any]=[], timeUnits:str='seconds') -> None:
		...
		""" 
		... 
	def __call__(self, audio:list[Any]) -> ndarray:
		"""compute
		Args:
			audio (list[Any]): the input audio signal. Defaults to . 
		Returns:
			frame (ndarray): the frames of the sliced input signal
		""" 
		... 
...
class StereoDemuxer(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, audio:ndarray) -> tuple[list[Any], list[Any]]:
		"""compute
		Args:
			audio (ndarray): the audio signal. Defaults to . 
		Returns:
			left (list[Any]): the left channel of the audio signal
			right (list[Any]): the right channel of the audio signal
		""" 
		... 
...
class StereoMuxer(_essentia.Algorithm): 
	def __init__(self, ) -> None:
		...
		""" 
		... 
	def __call__(self, left:list[Any], right:list[Any]) -> ndarray:
		"""compute
		Args:
			left (list[Any]): the left channel of the audio signal. Defaults to . 
			right (list[Any]): the right channel of the audio signal. Defaults to . 
		Returns:
...
class SuperFluxExtractor(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
			onsets (list[Any]): the onsets times
		""" 
		... 
...
class SuperFluxNovelty(_essentia.Algorithm): 
	def __init__(self, binWidth:int=3, frameWidth:int=2) -> None:
		...
		""" 
		... 
	def __call__(self, bands:ndarray) -> float:
		"""compute
		Args:
			bands (ndarray): the input bands spectrogram. Defaults to . 
		Returns:
...
class SuperFluxPeaks(_essentia.Algorithm): 
	def __init__(self, combine:float=30, frameRate:float=172, pre_avg:float=100, pre_max:float=30, ratioThreshold:float=16, threshold:float=0.05) -> None:
		...
		Args:
			combine (float): time threshold for double onsets detections (ms). Defaults to 30. Range (0,inf)
		...
		""" 
		... 
	def __call__(self, novelty:list[Any]) -> list[Any]:
		"""compute
		Args:
			novelty (list[Any]): the input onset detection function. Defaults to . 
		Returns:
			peaks (list[Any]): detected peaks' instants [s]
		""" 
		... 
...
class TCToTotal(_essentia.Algorithm): 
	...
	def __call__(self, envelope:list[Any]) -> float:
		"""compute
		Args:
			envelope (list[Any]): the envelope of the signal (its length must be greater than 1. Defaults to . 
		Returns:
...
class TempoTapDegara(_essentia.Algorithm): 
	def __init__(self, maxTempo:int=208, minTempo:int=40, resample:str='none', sampleRateODF:float=86.1328) -> None:
		...
		""" 
		... 
	def __call__(self, onsetDetections:list[Any]) -> list[Any]:
		"""compute
		Args:
			onsetDetections (list[Any]): the input frame-wise vector of onset detection values. Defaults to . 
		Returns:
...
class TonalExtractor(_essentia.Algorithm): 
	def __init__(self, frameSize:int=4096, hopSize:int=2048, tuningFrequency:float=440) -> None:
		...
		""" 
		... 
	def __call__(self, signal:list[Any]) -> tuple[float, list[Any], str, float, str, str, list[Any], ndarray, ndarray, str, str, float]:
		"""compute
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
			...
			chords_strength (list[Any]): See ChordsDetection algorithm documentation
			hpcp (ndarray): See HPCP algorithm documentation
			hpcp_highres (ndarray): See HPCP algorithm documentation
...
class TonicIndianArtMusic(_essentia.Algorithm): 
	def __init__(self, binResolution:float=10, frameSize:int=2048, harmonicWeight:float=0.85, hopSize:int=512, magnitudeCompression:float=1, magnitudeThreshold:float=40, maxTonicFrequency:float=375, minTonicFrequency:float=100, numberHarmonics:int=20, numberSaliencePeaks:int=5, referenceFrequency:float=55, sampleRate:float=44100) -> None:
		...
		Args:
			...
			magnitudeCompression (float): magnitude compression parameter (=0 for maximum compression, =1 for no compression). Defaults to 1. Range (0,1]
			magnitudeThreshold (float): peak magnitude threshold (maximum allowed difference from the highest peak in dBs). Defaults to 40. Range [0,inf)
			maxTonicFrequency (float): the maximum allowed tonic frequency [Hz]. Defaults to 375. Range [0,inf)
			minTonicFrequency (float): the minimum allowed tonic frequency [Hz]. Defaults to 100. Range [0,inf)
...
class Trimmer(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the input signal. Defaults to . 
		Returns:
			signal (list[Any]): the trimmed signal
		""" 
		... 
...
class TuningFrequencyExtractor(_essentia.Algorithm): 
	...
	def __call__(self, signal:list[Any]) -> list[Any]:
		...
		Args:
			signal (list[Any]): the audio input signal. Defaults to . 
		Returns:
			tuningFrequency (list[Any]): the computed tuning frequency
		""" 
		... 
...
class YamlInput(_essentia.Algorithm): 
	def __init__(self, filename:str='None', format:str='yaml') -> None:
...
class YamlOutput(_essentia.Algorithm): 
	...
	def __call__(self, pool:Pool) -> None:
		"""compute
		Args:
			pool (Pool): Pool to serialize into a YAML formatted file. Defaults to . 
		Returns:
None
		""" 
		... 
