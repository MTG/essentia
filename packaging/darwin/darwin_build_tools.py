...
if __name__ == '__main__':
    ...
    if copy_examples(essentia_root_dir,             # where essentia src lives
        ...
        print ("An error occurred while finalising osx package: error while copying examples")
        sys.exit(1)

    print ('* copying documentation to', target_dir)
    if copy_documentation(essentia_root_dir, target_dir):
        print ("An error occurred while finalising osx package: error while copying documentation")
        sys.exit(2)

    print ('* removing \".DS_Store\" files from', target_dir)
    if removeDSSTORE(target_dir):
        print ("An error occurred while finalising osx package: could not delete all \".DS_Store\" files.")
        sys.exit(3)

    print ('* copying tests from', essentia_root_dir, 'to', target_dir)
    if copy_python_tests(essentia_root_dir, target_dir):
        print ("An error occurred while finalising osx package: could not copy unittest files.")
        sys.exit(3)

    pkg_dir = PKG_DIR
    print ('* creating distribution pkg in', pkg_dir)
    if subprocess.call(['mkdir', pkg_dir]):
        print ("An error occurred while finalising osx package: could not create", pkg_dir)
        sys.exit(4)
    ...
    if subprocess.call(['mkdir', join(pkg_dir,'usr')]):
        print ("An error occurred while finalising osx package: could not create directory:", join(pkg_dir,'usr'))

    if subprocess.call(['cp', '-r', usr_dir, join(pkg_dir,'usr', 'local')]):
        print ("An error occurred while finalising osx package: could not move", usr_dir, "to", pkg_dir)
        sys.exit(5)

    if subprocess.call(['mv', library_dir, pkg_dir]):
        print ("An error occurred while finalising osx package: could not move", library_dir, "to", pkg_dir)
        sys.exit(6)

    if subprocess.call(['mv', target_dir, pkg_dir]):
        print ("An error occurred while finalising osx package: could not copy ", target_dir, "to", pkg_dir)
        sys.exit(7)
    ...
    if chown(usr_dir, 'root', 'wheel', True):
        print ("An error occurred while finalising osx package: could not change ownership of \"usr\" folder")
        sys.exit(8)
    ...
    if chown(library_dir, 'root', 'admin', True):
        print ("An error occurred while finalising osx package: error while changing ownership of \"Library\" folder")
        sys.exit(9)

    print ("* Building Package...")
    if buildPackage(PKG_DIR, essentia_root_dir):
        print ("An error occurred while finalising osx package: error while creating the package")
        ...
        print ("* in order to build the package you must use PackageMaker found in * /Developer")
        print ("*"*70)
        print ('\n')
        sys.exit(0)
    else:
        print ('\n')
        print ("*"*70)
        print ("* osx package build successful. You may find it in your root directory")
        print ("*"*70)
        print ('\n')
        sys.exit(0)
