...
def testWindowWholeRange(signals,signalsKey):
...
def testFrameSize(signals,signalsKey):
...
def testMaxFreq(signals,signalsKey):
...
def testJordiNL(signals,signalsKey):
    ...
    for skey in signalkey:
        ...
        for wtype in windowtypes:
            w = Windowing(type=wtype, normalized=win_normalized)
            for JordiNL in JordiNLList:
                hpcp.nonLinear = JordiNL
                title = "signal=" + signalname + " windowtype=" + wtype + " nonLinear=" + str(JordiNL)
                speaks = SpectralPeaks(maxPeaks=speaks_max)
                html.append(plotHPCP(signal, skey, hpcp, w, speaks, frameSize, hopSize, title))
                close('all')
...
def testWeigth(signals,signalsKey):
...
def testLPFilterMeanSpectrum(signals,signalsKey):
...
def testBPFilterMeanSpectrum(signals,signalsKey):
...
def testLPFilterFrameSpeaks(signals,signalsKey):
...
def testBPFilterFrameSpeaks(signals,signalsKey):
...
def testNormalizeWindow(signals,signalsKey):
...
def testNormalizeHPCP(signals,signalsKey):
