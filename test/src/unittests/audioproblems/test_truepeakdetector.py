...
class TestTruePeakDetector(TestCase):
    ...
    def testSinc(self):
        ...
        for offset in np.linspace(0, 1, its):
            ...
            _, processed = TruePeakDetector()(y.astype(np.float32))
    ...
    def testInvalidParam(self):
        ...
        self.assertConfigureFails(TruePeakDetector(), {'threshold': 1})
    ...
    def testDCblock(self):
        ...
        withoutDC = TruePeakDetector(blockDC=True,
                                     oversamplingFactor=oversamplingFactor)(signalWithDC.astype(np.float32))[0]
        withDC    = TruePeakDetector(blockDC=False,
                                     oversamplingFactor=oversamplingFactor)(signalWithDC.astype(np.float32))[0]

        assert(withDC.size == 0)
        assert(peakLoc * oversamplingFactor in withoutDC)
