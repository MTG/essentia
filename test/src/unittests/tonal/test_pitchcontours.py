...
class TestPitchContours(TestCase):
    ...
    def testEmptyFrames(self,sampleRate=defaultSampleRate):
        emptyPeakBins = [[],[]]
        emptyPeakSaliences = [[],[]]
        self.assertRaises(RuntimeError, lambda: PitchContours(hopSize=defaultHopSize,sampleRate=sampleRate)(emptyPeakBins, emptyPeakSaliences))

    def testUnequalInputs(self):
        # Suite of tests for unequal numbers of peaks in a frame, number of frames,etc.
        ...
        self.assertRaises(RuntimeError, lambda: PitchContours()(peakBins, peakSaliences))
        #unequal numbers of frames: 3 frame sin peakBins and 2 frames in peakSaliences
        peakBins = [zeros(4096), zeros(4096), zeros(4096)]
        peakSaliences =[zeros(4096), zeros(4096)]       
        self.assertRaises(RuntimeError, lambda: PitchContours()(peakBins, peakSaliences))
    
    def testDuration(self):
        # sanity to check that duration calculation is ok on artificial signal
        calculatedDuration = (len(testPeakBins)*defaultHopSize)/defaultSampleRate
        _, _, _, duration = PitchContours()(testPeakBins, testPeakSaliences)
