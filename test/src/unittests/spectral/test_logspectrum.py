...
class TestLogSpectrum(TestCase):
    def testRegression(self, frameSize=8192 + 1):
        ...
        audio = MonoLoader(
            filename=join(testdata.audio_dir, "recorded/vignesh.wav"), sampleRate=44100
        )()
        ...
        for frame in FrameGenerator(
            audio, frameSize=16384, hopSize=2048, startFromZero=True
        ):
    ...
    def testZero(self):
        ...
        while size >= 256:
            self.assertEqualVector(
                LogSpectrum(frameSize=size)(zeros(size))[0], zeros(256)
            )
            size = int(size / 2)
    ...
    def testInvalidParam(self):
        ...
        self.assertConfigureFails(LogSpectrum(), {"nOctaves": 0})
        self.assertConfigureFails(LogSpectrum(), {"nOctaves": 10})

    def testNOctavesParam(self):
        # TODO: for an specific frame_size check if the returned size is equal a reference sizes
        n_octaves = 1
        while n_octaves <= 9:
            self.assertEqualVector(
                LogSpectrum(frameSize=1024, nOctaves=n_octaves)(zeros(1024))[0],
                zeros(40 + 36 * (n_octaves - 1)),
            )
            n_octaves += 1
